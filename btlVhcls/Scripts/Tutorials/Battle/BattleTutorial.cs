...
public abstract class BattleTutorial : MonoBehaviour
{
    ...
    [SerializeField] protected SpriteFromRes mainCharacterTexture;

    protected bool allBotsDead;
    protected bool bonusLessonIsDone;
    protected int botKills;
    protected BonusItem closestBonus;
    ...
    {
        get { return GameData.IsGame(Game.Armada | Game.MetalForce) ? 2 : goalBotKills; }
    }
    ...
    protected Vector3 ArrowPosition
    {
        get
        {
            ...
            if (GameData.IsGame(Game.SpaceJet))
                return new Vector3(0.0f, 2.85f, 10.0f);
    ...
    private IEnumerator GetClosestBonus()
    {
        while (true)
        {
            if (BonusDispatcher.Instance.BonusItems.Count > 0)
            {
                //closestBonus = BonusDispatcher.Instance.bonusObjects[0];

                var minDist = float.MaxValue;

                foreach (var bonus in BonusDispatcher.Instance.BonusItems)
                {
                    if (bonus == null || !bonus.IsMapBonus)
                        continue;
    ...
    public void SetupCharacterTexture()
    {
        ...
        if (GameData.IsGame(Game.MetalForce | Game.BattleOfWarplanes | Game.WingsOfWar))
        {
            ...
            {
                case Localizer.LocalizationLanguage.Russian: mainCharacterTexture.SetTexture("tutorCharacter_battle"); break;
                default: mainCharacterTexture.SetTexture("tutorCharacterMan_battle"); break;
            }
        }
    }
