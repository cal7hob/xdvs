...
public class TutorialsController : MonoBehaviour
{
    ...
    public Dictionary<int, GameObject> tutorialGroups = new Dictionary<int, GameObject>();
    ...
    void Awake()
    {
        ...
        Dispatcher.Subscribe(EventId.ModuleBought, OnModuleBought);
    ...
    void OnDestroy()
    {
        ...
        Dispatcher.Unsubscribe(EventId.ModuleBought, OnModuleBought);
        Dispatcher.Unsubscribe(EventId.VehicleBought, OnVehicleBought);
    }

    private static void OnCamoBought(EventId id, EventInfo info)
    {
        if (!ProfileInfo.accomplishedTutorials[Tutorials.buyCamouflage]) {
    ...
    {
        if (!ProfileInfo.accomplishedTutorials[Tutorials.vehicleUpgrade]) {
    ...
    {
        if (!ProfileInfo.accomplishedTutorials[Tutorials.buyVehicle]) {
    ...
    private void Init(EventId id = 0, EventInfo info = null)
    {
        ...
        var tutorialsHolder = (GameObject) Resources.Load(Tutorial.GetHangarPrefabPath(Tutorial.TutorPrefab.TutorialHolder));
        ...
        switch (currentTutorial) {
            ...
            case Tutorials.enterName:
                // Skip tutor if player has or declines nick
                if (ProfileInfo.nickEntered || ProfileInfo.nickRejected) { 
                    ProfileInfo.accomplishedTutorials[Tutorials.enterName] = true;
                    Init (id, info);
                    return;
                }
            ...
            case Tutorials.vehicleUpgrade:
                if (GoToModuleShopTutorial.HasUpgradedModule) { // Skip tutor if player has module
                    ProfileInfo.accomplishedTutorials[Tutorials.vehicleUpgrade] = true;
                    Init (id, info);
                    return;
                }
            ...
            case Tutorials.buyCamouflage:
                if (GoToPatternShopTutorial.HasCamouflage) { // Skip tutor if player has camouflage
                    ProfileInfo.accomplishedTutorials[Tutorials.buyCamouflage] = true;
                    Init (id, info);
                    return;
                }
