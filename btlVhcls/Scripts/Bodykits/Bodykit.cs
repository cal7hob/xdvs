...
public class Bodykit : IShopItem, IStatGainer
{
    ...
    protected Bodykit(PatternEntity patternEntity)
    {
        id = patternEntity.id;
        isHidden = patternEntity.isHidden;
        isVip = patternEntity.isVip;
        availabilityLevel = patternEntity.availabilityLevel;
        lifetime = patternEntity.lifetime;
        damageGain = patternEntity.damageGain;
        rocketDamageGain = patternEntity.rocketDamageGain;
        speedGain = patternEntity.speedGain;
        armorGain = patternEntity.armorGain;
        rofGain = patternEntity.rofGain;
        ircmRofGain = patternEntity.ircmRofGain;
        pricesToGroups = patternEntity.pricesToGroups;
    }
        ...
        if (damageGain > 0)
        {
            status
                += string.Format(
                    ...
                    Convert.ToInt32(damageGain * 100),
        ...
        if (rocketDamageGain > 0)
        {
            status
                += string.Format(
                    "{0}+{1}% {2}",
                    status.Length > 0 ? Environment.NewLine : "",
                    Convert.ToInt32(rocketDamageGain * 100),
                    Localizer.GetText("ForDamageGain"));
