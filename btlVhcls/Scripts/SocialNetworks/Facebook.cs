...
#pragma warning restore 67
        ...
        public SocialPlatform Platform { get { return SocialPlatform.Facebook; } }
        ...
        private Dictionary<string, SocialPrice> prices;

        public SocialPrice GetPriceById (string item_id)
        {
            if (prices == null) {
                prices = new Dictionary<string, SocialPrice> (GameData.socialPrices.Count);
                foreach (var socialPrice in GameData.socialPrices) {
                    ...
                    prices[socialPrice.Key] = new SocialPrice (price, SocialCurrency.Dollar);
                }
            }
            if (!prices.ContainsKey(item_id)) {
                Debug.LogWarningFormat("[Facebook] Can't find price for {0}", item_id);
                return null;
            }
        ...
        {
            var price = GetPriceById(item_id);
            if (price == null)             {
                return "";
            }
        ...
        private void CheckAppPage(Action<bool> onComplete = null)
        {
            ...
            FB.API("/me/likes/"+appPageId, HttpMethod.GET, delegate(IGraphResult likes)
            {
                ...
                var info = new SocialNetworkGroupInfo(
                    ...
                    appPageId,
                    Application.productName + " App Page",
