...
public class AfterBattleIncomeFrame : MonoBehaviour
{
    ...
    public void Init(bool isVipFrame)
    {
        ...
        if (exp > 0)
            data.Add(RowType.Experience, new Data(exp, useCustomRowPrefabs ? null : "Experience"));
        if(br.gold > 0)
            data.Add(RowType.Gold, new Data(br.gold, useCustomRowPrefabs ? null : "gold"));
        if(earnedSilver > 0)
            data.Add(RowType.Silver, new Data(earnedSilver, useCustomRowPrefabs ? null : "silver"));
        if(br.fuel > 0)
            data.Add(RowType.Fuel, new Data(br.fuel, useCustomRowPrefabs ? null : "fuel"));
        ...
        if (!br.isAllQuestsCompleted)
        {
            if ((br.quest != null) && br.quest.isComplete)
            {
                data.Add(RowType.Quest, new Data(
                    ...
                    useCustomRowPrefabs ? null : br.quest.GetQuestReward(isVipFrame).currency.ToString().ToLower(),
                    "lblFromQuest"));
                ...
                if (br.questAll.value > 0) {
                    data.Add(RowType.AllQuests, new Data(
                        ...
                        useCustomRowPrefabs ? null : QuestsInfo.GetAllQuestsReward(isVipFrame).currency.ToString().ToLower(),
                        "lblFrom3Quest"));
                }
            }
        }
