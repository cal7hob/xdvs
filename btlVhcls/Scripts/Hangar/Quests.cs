    ...
    public bool CheckQuestComplete()
    {
        switch (this.type)
        {
            ...
            case Type.Collect:
                ...
                return isComplete = BattleStatisticsManager.BattleStats[BattleStatsDictionaryKey] >= CompleteCount;
            case Type.Mileage:
                if (!BattleStatisticsManager.BattleStats.ContainsKey("Mileage"))
                {
                    DT.LogError("CheckQuestComplete(). BattleStatisticsManager.BattleStats not contains {0}", "Mileage");
                    return false;
                }
                return isComplete = BattleStatisticsManager.BattleStats["Mileage"] >= CompleteCount;
            case Type.Revenge:
                return isComplete = BattleStatisticsManager.isRevengeDone;
