...
public class VehicleUpgrades
{
    ...
    public void SetCamouflageById(int camoId)
    {
        camouflageId = camoId;

        if (camoId != 0 && OwnedCamouflages.All(camo => camo.id != camoId))
            ownedCamouflages.Add(PatternPool.Instance.GetItemById(camoId).ToPurchased());
    }

    public void SetDecalById(int decalId)
    {
        this.decalId = decalId;

        if (decalId != 0 && OwnedDecals.All(decal => decal.id != decalId))
            ownedDecals.Add(DecalPool.Instance.GetItemById(decalId).ToPurchased());
    }
