...
public class PlayerInfo : MonoBehaviour
{
    ...
    {
        GUIPager.SetActivePage("PlayerInfo", true);
            ...
            = experience != -1
                ? string.Format(
                    ...
                    Mathf.Clamp(
                        ...
                        1,
                        50),
                    GameData.IsGame(Game.BattleOfHelicopters) ? "" : Localizer.GetText("Level"))
                : string.Empty;
        ...
        bool countRocketShots = GameData.IsGame(Game.BattleOfWarplanes | Game.WingsOfWar | Game.BattleOfHelicopters | Game.BlowOut);

        battlesCountValueLabel.text = battleStats.ContainsKey("battlesCount") ?  battleStats["battlesCount"].ToString() : "0";
        maxKillsInARowValueLabel.text = battleStats.ContainsKey("maxKillsInARow") ? battleStats["maxKillsInARow"].ToString() : "0";

        string key = countRocketShots ? "totalShootsSaclos" : "totalShoots";
        shootsValueLabel.text = battleStats.ContainsKey(key) ? battleStats[key].ToString() : "0";
        key = countRocketShots ? "totalHitsSaclos" : "totalHits";
        hitsValueLabel.text = battleStats.ContainsKey(key) ? battleStats[key].ToString() : "0";
        key = countRocketShots ? "overralAccuracySaclos" : "overralAccuracy";
        accuracyValueLabel.text = battleStats.ContainsKey(key) ? battleStats[key].ToString() : "0";

        fragsValueLabel.text = battleStats.ContainsKey("totalFrags") ? battleStats["totalFrags"].ToString() : "0";
        mileageValueLabel.text = battleStats.ContainsKey("totalMileage") ? battleStats["totalMileage"].ToString() : "0";
        deathsValueLabel.text = battleStats.ContainsKey("totalDeaths") ? battleStats["totalDeaths"].ToString() : "0";
    }
