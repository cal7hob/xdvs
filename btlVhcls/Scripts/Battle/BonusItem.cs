...
public class BonusItem : MonoBehaviour
{
    ...
    public VehicleEffect tankEffect;
    ...
    public void OnPhotonInstantiate(PhotonMessageInfo messageInfo)
    {
        ...
        SetShadow();
        if (tankEffect != null)
            tankEffect.SetSource(bonusType);
    ...
    void OnDestroy()
    {
        ...
        if (BattleConnectManager.IsMasterClient && isMapBonus)
            PhotonNetwork.room.SetCustomProperties(new Hashtable { { "bc", (int)PhotonNetwork.room.CustomProperties["bc"] - 1 } });

        if (BattleConnectManager.IsMasterClient)
        {
            Dispatcher.Send(EventId.BonusDestroyed, new EventInfo_II((int)bonusType, photonView.viewID));
        }
    ...
    void OnTriggerEnter(Collider other)
    {
        ...
        if (otherLayer == BattleController.ParallelWorldLayer
            || (otherLayer != BattleController.PlayerLayer && !BattleConnectManager.IsMasterClient))
            return;

        VehicleController veh = other.transform.GetComponentInParent<VehicleController>();
        if (veh == null || !veh.IsAvailable || veh.IsCrashing)
            return;

        if (tankEffect != null && tankEffect.Type != VehicleEffect.ParameterType.None)
            veh.ApplyEffect(tankEffect); //Бонус с моментальным эффектом
