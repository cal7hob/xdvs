...
public class ThrottleLevel : MonoBehaviour
{
    ...
    [SerializeField] private InterfaceExtensions.ConditionHelper platformSpecificPositionChanger;
    ...
    void Awake()
    {
        ...
        if (platformSpecificPositionChanger)
            platformSpecificPositionChanger.State = System.Convert.ToInt32(!BattleGUI.IsTargetPlatformForShowingJoysticks);
    ...
    void Start()
    {
        wrapper.gameObject.SetActive(true);// must to always be shown to indicate speed Level
    ...
    protected virtual void OnMyTankRespawned(EventId id, EventInfo ei)
    {
        ResetThumbPosition();
        UpdateValue();
        UpdateFiller();
    ...
    protected virtual void ResetThumbPosition()
    {
