...
public class FloatSpeedXYJoystick : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (BattleSettings.OnScreen || StatTable.OnScreen || (BattleChatCommands.Instance && BattleChatCommands.Instance.IsActive))
            return;
    ...
    #region  Методы для отображения мёртвых зон для джойстика поворота башни. Включаем для откладки мёртвых зон. Не удалять
    ...
    //void OnGUI()
    //{
    //    foreach (var rect in joyAreaException)
    //        UnityEditor.EditorGUI.DrawRect(reYcoordforOnGUI(rect), new Color(0, 0, 0.4f, 0.5f));
    //}
    #endregion
}
