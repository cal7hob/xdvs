...
public class StatTable : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        Dispatcher.Subscribe(EventId.PhotonJoinedRoom, Init);
        ...
        Dispatcher.Subscribe(EventId.TroubleDisconnect, OnTroubleDiconnect);
        Dispatcher.Subscribe(EventId.TeamScoreChanged, OnTeamScoreChanged);
    }
    ...
    void OnDestroy()
    {
        ...
        Dispatcher.Unsubscribe(EventId.PhotonJoinedRoom, Init);
        Dispatcher.Unsubscribe(EventId.MyTankRespawned, OnMyTankRespawned);
        Dispatcher.Unsubscribe(EventId.BeforeReconnecting, OnTroubleDiconnect);
        Dispatcher.Unsubscribe(EventId.TroubleDisconnect, OnTroubleDiconnect);
        Dispatcher.Unsubscribe(EventId.TeamScoreChanged, OnTeamScoreChanged);
    }
    ...
    public static void Hide()
    {
        ...
        Dispatcher.Send(EventId.OnStatTableChangeVisibility, new EventInfo_B(false));
        ...
        scheduledShow = () =>
            {
                ...
                Dispatcher.Send(EventId.OnStatTableChangeVisibility, new EventInfo_B(true));
            };
    ...
    private int FillRowsSpecial(List<PlayerStat> statistics, ref List<StatTableRow> rows, int count, ref GameObject[] fillers, bool hasOwnStat)
    {
        ...
        foreach (PlayerStat stat in statistics)
        {
            ...
            row.IsOffender = (stat.playerId == BattleController.LastOffender);
    ...
    {
        var price = new ProfileInfo.Price(GameData.hastenPrice, ProfileInfo.PriceCurrency.Gold);
        if (!ProfileInfo.IsPlayerVip) {
            if (!ProfileInfo.CanBuy (price)) return;
            ProfileInfo.WriteoffBalance (price);
        }

        counter.MoveToEnd();
        Http.Manager.BattleServer.BuyHastenRespawn ();
    ...
    private void OnBuyBonusClick(tk2dUIItem item)
    {
        ...
        var price = new ProfileInfo.Price(bought.Price, ProfileInfo.PriceCurrency.Gold);
