...
public abstract class ThirdPartyAdsService : MonoBehaviour, IAdsService
{
    ...
    public event Action OnRewardedVideoFailedToLoad;
    ...
    public abstract bool HasRewardedVideos { get;}
    ...
    protected virtual void RaiseOnRewardedVideoLoaded()
    {
        var handler = OnRewardedVideoLoaded;
        if (handler != null) handler();
    }

    protected virtual void RaiseOnRewardedVideoFailedToLoad()
    {
        var handler = OnRewardedVideoFailedToLoad;
        if (handler != null) handler();
    }

    protected virtual void RaiseOnRewardedVideoShown()
    {
        var handler = OnRewardedVideoShown;
        if (handler != null) handler();
    }

    protected virtual void RaiseOnRewardedVideoFinished()
    {
        var handler = OnRewardedVideoFinished;
        if (handler != null) handler();
    }

    protected virtual void RaiseOnRewardedVideoClosed()
    {
        var handler = OnRewardedVideoClosed;
        if (handler != null) handler();
    }
}
