...
public class NotificationsManager : MonoBehaviour, IQueueablePage
{
    ...
    {
        Instance = this;
        Dispatcher.Subscribe(EventId.AfterHangarInit, ProcessNotifications);
        Dispatcher.Subscribe(EventId.ProfileInfoLoadedFromServer, ProcessNotifications);
        OnNotificationsChanged += NotificationsChangedHandler;
    }

    private void OnDestroy()
    {
        Dispatcher.Unsubscribe(EventId.AfterHangarInit, ProcessNotifications);
        Dispatcher.Unsubscribe(EventId.ProfileInfoLoadedFromServer, ProcessNotifications);
        OnNotificationsChanged -= NotificationsChangedHandler;
        Instance = null;
    }
    ...
    public void NotificationsChangedHandler()
    {
        ...
        {
            if (GUIPager.ActivePageName != "NotificationWindow")
            {
                // TODO: решить что делать с этим. если переходим на страницу, например, в Банк.
                GUIPager.EnqueuePage("NotificationWindow", false);
            }
            else
                BeforeActivation();
        }
    ...
    public static void TestFromEditor()
    {
        ...
        Dispatcher.Send(EventId.ProfileInfoLoadedFromServer, null);
    }
