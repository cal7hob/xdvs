...
public static class HelpTools
{
    ...
    [MenuItem("HelpTools/FontNormalizer/BO")]
    public static void FontNormalizerBO() { FontNormalizer(Interface.BlowOut); }

    [MenuItem("HelpTools/FontNormalizer/BW")]
    public static void FontNormalizerBW() { FontNormalizer(Interface.BattleOfWarplanes); }

    [MenuItem("HelpTools/FontNormalizer/WW")]
    public static void FontNormalizerWW() { FontNormalizer(Interface.WingsOfWar); }
    ...
    [MenuItem("HelpTools/FontNormalizer/MF")]
    public static void FontNormalizerMF() { FontNormalizer(Interface.MetalForce); }
    ...
    */
    ...
    public static void ImportComponentsPatternEntity(PatternEntity[] patternEntities, JSONObject data, string key)
    {
        Debug.Log("ImportComponentsPatternEntity");

        JsonPrefs jsonData = new JsonPrefs(data);

        Dictionary<int, PatternEntity> componentsDict = new Dictionary<int, PatternEntity>(patternEntities.Length);

        foreach (PatternEntity entity in patternEntities)
        {
            int ind = entity.name.IndexOf('_');

            if (ind == -1)
            {
                DT.LogWarning("No id specified in object name ({0})", entity.name);
                componentsDict[0] = entity;
                continue;
            }

            int id;

            if (!int.TryParse(entity.name.Substring(0, ind), out id) &&
                !int.TryParse(entity.name.Substring(ind + 1), out id))
            {
                continue;
            }

            componentsDict.Add(id, entity);
        }
    ...
    public static void SetRotationToAllObjectsInCollection(IEnumerable<Transform> collection, Quaternion rotation)
    {
        if (collection != null)
    ...
    /// <summary>
    /// Determine the signed angle between two vectors, with normal
    ...
    public static void CreateFolderRecursively(string path)
    {
        ...
        for (int i = 0; i < folders.Length; i++)
        {
            ...
            if (!AssetDatabase.IsValidFolder(currentPath + folder))
                AssetDatabase.CreateFolder(currentPath.TrimEnd('/'), folder);
