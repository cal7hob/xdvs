...
namespace AssetBundles
{
    ...
    public class AssetBundleManager : MonoBehaviour
    {
        ...
        {
            if (absolutePath.StartsWith("/") || absolutePath.Substring(1, 1) == ":")
            {
        ...
        public static void SetSourceAssetBundleAbsolteDirectory(string absolutePath)
        {
            if (!absolutePath.EndsWith("/"))
            {
                absolutePath += "/";
        ...
        static public IEnumerator CachingBundles ()
        {
#if UNITY_EDITOR
            // If we're in Editor simulation mode, we don't need the manifest assetBundle.
            if (SimulateAssetBundleInEditor)
            ...
            foreach (var bundleName in bundles) {
                string bundleBaseDownloadingURL = GetAssetBundleBaseDownloadingURL(bundleName);
        ...
        static public string FindAssetBundle (string startsWith)
        {
            string[] bundles;
#if UNITY_EDITOR
            if (SimulateAssetBundleInEditor)
