...
public static class ProfileInfo
{
    ...
    public static int dailyBonusMissed;
    public static bool dailyBonusIsAvailable;
    ...
    public static Dictionary<int, ConsumableProfileData> consumableInventory = new Dictionary<int, ConsumableProfileData>();
    public static List<int> consumableInventoryPanelItems = new List<int>();//индекс -> слот, элемент -> ид_расходки
    public static List<int> superWeaponsInventoryPanelItems = new List<int>();//индекс -> слот, элемент -> ид_расходки
    ...
    public static Dictionary<string, int> roulettParams = new Dictionary<string, int>();
    ...
    private static int autoAimingType = (int)AutoAimingType.DefaultAutoAiming;
    ...
    #endregion
    #region RewardedVideoController
    public static bool mustToShowBtnGetDoubleRewardOnAfterBattleScreen = false;
    public static bool mustToShowBtnModuleDeliveryForAdsViewing = false;
    public static int timeToShowBtnModuleDeliveryForAdsViewing = 0;
    public static string adsPlatform = "ChartBoost";
    public static bool IsNeededToShowBtnModuleDeliveryForAdsViewing
    {
        get
        {
            if (Shop.CurrentVehicle == null || Shop.CurrentVehicle.Upgrades == null || Shop.CurrentVehicle.Upgrades.awaitedModule == TankModuleInfos.ModuleType.None || !RewardedVideoController.ServiceHasAds)
                return false;
            var upgradeRemainingTimeSec = Shop.CurrentVehicle.Upgrades.moduleReadyTime - GameData.CurrentTimeStamp;
            return mustToShowBtnModuleDeliveryForAdsViewing && (upgradeRemainingTimeSec <= timeToShowBtnModuleDeliveryForAdsViewing);
        }
    }
    public static bool IsNeededToShowBtnGetDoubleRewardForAdsViewing
    {
        get
        {
            return mustToShowBtnGetDoubleRewardOnAfterBattleScreen && !IsPlayerVip && RewardedVideoController.ServiceHasAds;
        }
    }
    #endregion
    ...
    public static AutoAimingType AutoAimingType
    {
        get { return (AutoAimingType)autoAimingType; }
        set { autoAimingType = (int)value; }
    }
    ...
    public class Price
    {
        ...
        private static Dictionary<Interface, Dictionary<PriceCurrency, Color>> moneyColors = new Dictionary<Interface, Dictionary<PriceCurrency, Color>>()
        {
            ...
            {Interface.WingsOfWar, new Dictionary<PriceCurrency, Color>()
                {
                    {PriceCurrency.Gold, new Color32(255, 194, 95, 255)},
                    {PriceCurrency.Silver, new Color32(206, 206, 206, 255)},
                }
            },
            {Interface.MetalForce, new Dictionary<PriceCurrency, Color>()
                {
                    {PriceCurrency.Gold, new Color32(191, 151, 107, 255)},//BF976BFF
                    {PriceCurrency.Silver, new Color32(184, 177, 158, 255)},//b8b19e
                }
            },
        };
        ...
        {
            if (dict == null)
                return new Price(0, PriceCurrency.Silver);
            JsonPrefs data = new JsonPrefs(dict);
            return new Price(data.ValueInt("value"), dict.ExtractOrDefault("currency", PriceCurrency.Silver));
    }

        public static Price Gold(int amount)
        {
            return new Price(amount, PriceCurrency.Gold);
        }

        public static Price Silver(int amount)
        {
            return new Price(amount, PriceCurrency.Silver);
        }
        ...
        public string LocalizedValue { get { return MiscTools.GetCultureSpecificFormatOfNumber(value); } }
    ...
    public static double Fuel
    {
        ...
        set
        {
            ...
            Dispatcher.Send(EventId.FuelUpdated, new EventInfo_I((int)fuel));
        }
    }
    ...
    public static int MaxFuel
    {
        ...
        set
        {
            ...
            Dispatcher.Send(EventId.MaxFuelUpdated, new EventInfo_I(maxFuel));
        }
    }
    ...
    static ProfileInfo () {
        accomplishedTutorials = new Dictionary<Tutorials, bool> ();
    ...
    public static bool ReplenishBalance(Price price)
    {
        if (IsBattleTutorial) return false; // Не начисляем ничего в боевом туторе
    ...
    public static bool WriteoffBalance (Price price)
    {
...
//        dict["LastFriendsInvite"] = (double)lastFriendsInvite;
        ...
        dict["SuperWeaponsInventoryPanelItems"] = superWeaponsInventoryPanelItems;
        ...
        dict["AutoAimingType"] = autoAimingType;
    ...
    public static bool ApplyLoadedData(Dictionary<string, object> data, long timestamp)
    {
        ...
        autoAimingType = prefs.ValueInt("AutoAimingType", (int)AutoAimingType.DefaultAutoAiming);
        ...
        dailyBonusDay = prefs.ValueInt ("DailyBattleBonus/day", 0);
        dailyBonusMissed = prefs.ValueInt ("DailyBattleBonus/missed", 0);
        dailyBonusIsAvailable = prefs.ValueBool ("DailyBattleBonus/isAvailable", false);
        ...
        if (prefs.Contains("SuperWeaponsInventoryPanelItems"))
            LoadSuperWeaponsInventoryPanelItemsFromDict(prefs.ValueObjectList("SuperWeaponsInventoryPanelItems"));

        if (prefs.Contains("WheelOfFortune"))
            LoadRoulettParamsFromDict(prefs.ValueObjectDict("WheelOfFortune"));
        ...
        if (prefs.Contains("Ads"))
            LoadAdsParametersFromDic(prefs.ValueObjectDict("Ads"));
    ...
    public static void ApplyProfileChanges(Dictionary<string, object> data)
    {
        ...
        if (data.ContainsKey("IsFuelForInviteAvailable"))
        {
            ...
            if (lastSate != isFuelForInviteAvailable)
            {
                if (null != SocialSettings.OnLastInviteChanged)
                {
                    ...
                    {
                        Dispatcher.Send(EventId.FriendInviteSuccess, null);
                    }
                }
            }
        }
        ...
        dailyBonusDay = prefs.ValueInt ("DailyBattleBonus/day", dailyBonusDay);
        dailyBonusMissed = prefs.ValueInt ("DailyBattleBonus/missed", dailyBonusMissed);
        dailyBonusIsAvailable = prefs.ValueBool("DailyBattleBonus/isAvailable", dailyBonusIsAvailable);
        ...
        #region Bank Offers limits
        if (data.ContainsKey ("IsGoldDiscountLimitReached") ||
            data.ContainsKey ("IsSilverDiscountLimitReached") || 
            data.ContainsKey ("IsVipDiscountLimitReached"))
        {
    ...
    private static void NotifyAboutChanges(bool tanksChanged)
    {
        ...
        if (HangarController.Instance && HangarController.Instance.IsInitialized)
            Dispatcher.Send(EventId.ProfileInfoLoadedFromServer, new EventInfo_SimpleEvent());
    }
    ...
    private static void LoadConsumableInventoryPanelItemsFromDict(List<object> list)
    {
        ...
        for (int i = 0; i < GameData.consumablesInventoryCapacity; i++)
            consumableInventoryPanelItems.Add(i < list.Count ? Convert.ToInt32(list[i]) : ConsumablesInventoryPanel.EMPTY_CELL_ID);
    ...
    private static void LoadSuperWeaponsInventoryPanelItemsFromDict(List<object> list)
    {
        superWeaponsInventoryPanelItems.Clear();
        for (int i = 0; i < GameData.consumablesSuperWeaponInventoryCapacity; i++)
            superWeaponsInventoryPanelItems.Add(i < list.Count ? Convert.ToInt32(list[i]) : SuperWeaponsInventoryPanel.EMPTY_CELL_ID);
    }

    private static void LoadRoulettParamsFromDict(Dictionary<string,object> dic)
    {
        var prefs = new JsonPrefs(dic);
        roulettParams = new Dictionary<string, int>()
        {
            { "attempts", prefs.ValueInt("attempts", 0)},
            { "sector", prefs.ValueInt("sector", 0)},
            { "timeout", prefs.ValueInt("timeout", 0)}
        };
    }
    ...
    public static void LoadDefaults()
    {
        ...
        autoAimingType = (int) AutoAimingType.DefaultAutoAiming;
        isHideMyFlag = false;
        isSliderControl = true;
        isVoiceDisabled = false;
        LoadTutorials ();
        LoadRoulettParamsFromDict(null);
    }
    ...
    private static void LoadInventory(Dictionary<string, object> consumablesDict)
    {
        ...
        foreach (Dictionary<string, object> consumableDict in consumablesDict.Values)
        {
            try
            {
                ...
                consumableInventory.Add(id, new ConsumableProfileData(consumableDict));
            }
            catch (Exception)
            {
    ...
    {
        return consumableInventory != null && consumableInventory.ContainsKey(id) && consumableInventory[id].count > 0;
    }

    private static void LoadAdsParametersFromDic(Dictionary<string, object> dic)
    {
        JsonPrefs prefs = new JsonPrefs(dic);
        mustToShowBtnGetDoubleRewardOnAfterBattleScreen = prefs.ValueBool("afterBattleScreen/show");
        mustToShowBtnModuleDeliveryForAdsViewing = prefs.ValueBool("deliveryScreen/show");
        timeToShowBtnModuleDeliveryForAdsViewing = prefs.ValueInt("deliveryScreen/timeToShow");
