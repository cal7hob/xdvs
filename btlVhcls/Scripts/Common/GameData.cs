...
public enum Game : ulong
{
    ...
    BlowOut                             = 0x00010000,   // 0000 0000 0001 0000 0000 0000 0000
    ...
    MetalForce                          = 0x00000004,   // 0000 0000 0000 0000 0000 0000 0100       
    WingsOfWar                          = 0x00000008,   // 0000 0000 0000 0000 0000 0000 1000          
    ...
    BlowOutAmazon = BlowOut | AmazonBuild,
    BattleOfHelicoptersAmazon = BattleOfHelicopters | AmazonBuild,
    ArmadaAmazon = Armada | AmazonBuild,
    MetalForceAmazon = MetalForce | AmazonBuild,
    WingsOfWarAmazon = WingsOfWar | AmazonBuild,
    ...
    Tanks = IronTanks | FutureTanks | ToonWars | Armada,
    Cars = BlowOut | MetalForce,
    Aircrafts = BattleOfWarplanes | WingsOfWar,
    All = Tanks | Cars | SpaceJet | Aircrafts | BattleOfHelicopters,
}
...
public class GameData : MonoBehaviour
{
    ...
    public const int TUTORIAL_VEHICLE_ID = 15;
    public const int TUTORIAL_VEHICLE_ID_AR = 9;
    public const int TUTORIAL_VEHICLE_ID_MF = 8;
    public const int TUTORIAL_CAMO_ID = 0; // Пока применяется только в MF.
    public const int TUTORIAL_CAMO_ID_MF = 14;
    public const string UNKNOWN_FLAG_NAME = "xx";
    public const int LINK_SUGGESTION_BATTLES_COUNT = 3;
    ...
    [SerializeField]
    private UnityEngine.Object rewiredInputManager;
    ...
    public static int consumablesInventoryCapacity = 3;
    public static int consumablesSuperWeaponInventoryCapacity = 1;
    ...
    public const string CONSUMABLES_SPRITE_FRAMED_VERSION_SUFFIX = "_square";
    ...
    #region DailyBonus
    public static List<DailyBonusInfo> dailyBonusInfos;
    #endregion

    #region Fortune Page (WheelOfFortune)
    public static List<RoulettItemInfo> roulettItems;
    public static int roulettAttemptsCount;
    #endregion
    ...
    public static string GetInterfaceShortName(Interface customInterface)
    {
        switch(customInterface)
        {
            ...
            case Interface.SpaceJet: return "SJ";
            case Interface.BattleOfWarplanes: return "BW";
            case Interface.WingsOfWar: return "WW";
            case Interface.BattleOfHelicopters: return "BH";
            case Interface.Armada: return "AR";
            case Interface.MetalForce: return "MF";
            case Interface.BlowOut: return "BO";
    ...
    public static int CurrentTutorialVehicleId
    {
        get
        {
            if (IsGame(Game.Armada))
                return TUTORIAL_VEHICLE_ID_AR;

            if (IsGame (Game.MetalForce))
                return TUTORIAL_VEHICLE_ID_MF;

            return TUTORIAL_VEHICLE_ID;
        }
    }

    public static int CurrentTutorialCamoId
    {
        get
        {
            if (IsGame(Game.MetalForce))
                return TUTORIAL_CAMO_ID_MF;

            return TUTORIAL_CAMO_ID;
        }
    }
    ...
    public static Interface CurInterface
    {
        get
        {
            ...
            if (!EditorApplication.isPlaying)
            {
                ...
                if (UnityEditor.PlayerSettings.applicationIdentifier == androidBundleIds[Game.WingsOfWar] ||
                        UnityEditor.PlayerSettings.applicationIdentifier == androidBundleIds[Game.WingsOfWarAmazon])
                    return Interface.WingsOfWar;
                if (UnityEditor.PlayerSettings.applicationIdentifier == androidBundleIds[Game.BlowOut] ||
                        UnityEditor.PlayerSettings.applicationIdentifier == androidBundleIds[Game.BlowOutAmazon])
                    return Interface.BlowOut;
                ...
                if (UnityEditor.PlayerSettings.applicationIdentifier == androidBundleIds[Game.MetalForce] ||
                    UnityEditor.PlayerSettings.applicationIdentifier == androidBundleIds[Game.MetalForceAmazon])
                    return Interface.MetalForce;
            ...
            if (GameData.IsGame(Game.FutureTanks))
                ...
                return Interface.SpaceJet;
            else if (GameData.IsGame(Game.BlowOut))
                return Interface.BlowOut;
    ...
    public static Dictionary<Game, string> androidBundleIds = new Dictionary<Game, string>() {
        ...
        {Game.WingsOfWar,           "com.extremedevelopers.wingsofwar"},
        {Game.BlowOut,              "com.extremedevelopers.blowout"},
        {Game.BattleOfHelicopters,  "com.extremedevelopers.battleofhelicopters"},
        {Game.Armada,               "com.extremedevelopers.armada"},
        {Game.MetalForce,           "com.extremedevelopers.metalforce"},
        {Game.IronTanksAmazon,      "com.extremedevelopers.irontanksam"},
        {Game.FutureTanksAmazon,    "com.extremedevelopers.futuretanksam"},
        {Game.ToonWarsAmazon,       "com.extremedevelopers.toonboomam"},
        {Game.SpaceJetAmazon,       "com.extremedevelopers.spacejet"},
        {Game.BattleOfWarplanesAmazon,"com.extremedevelopers.battleofwarplanes"},
        {Game.WingsOfWarAmazon,     "com.extremedevelopers.wingsofwar"},
        {Game.BlowOutAmazon,        "com.extremedevelopers.blowout"},
        {Game.BattleOfHelicoptersAmazon,"com.extremedevelopers.battleofhelicopters"},
        {Game.ArmadaAmazon,         "com.extremedevelopers.armada"},
        {Game.MetalForceAmazon,     "com.extremedevelopers.metalforce"},
        
    };

    public static Dictionary<Interface, string> assetsProjectFolder = new Dictionary<Interface, string>
    {
        { Interface.IronTanks, "01_IRON_TANKS"},
        { Interface.FutureTanks, "02_FUTURE_TANKS"},
        { Interface.ToonWars, "03_TOON_WARS"},
        { Interface.SpaceJet, "04_SPACE_JET"},
        { Interface.BattleOfWarplanes, "05_AIRCRAFTS"},
        { Interface.WingsOfWar, "09_WINGS_OF_WAR"},
        { Interface.BattleOfHelicopters, "06_HELICOPTERS"},
        { Interface.Armada, "07_ARMADA"},
        { Interface.MetalForce, "08_METAL_FORCE"},
        { Interface.BlowOut, "10_BLOW_OUT"},
    };

    private static GameMode gameMode = GameMode.Deathmatch;
    ...
    public static bool AllowBackgroundConnection { get; private set; }
    ...
    {
            return string.Format("scnh_{0}_{1}",
    ...
    public static bool DeviceHasHyroscope { get { return SystemInfo.deviceType == DeviceType.Handheld; } }//Пока так. (раз в юнити нет такого свойства)
    ...
    void Awake()
    {
        ...
        Dispatcher.Subscribe(EventId.AfterHangarInit, OnHangarInit);
        ...
        Instantiate(rewiredInputManager, transform).name = "Rewired Input Manager";
        ...
        Dispatcher.Subscribe (EventId.ProfileServerChoosed, StartInit, 1);
    }
    ...
    void OnDestroy()
    {
        ...
        Dispatcher.Unsubscribe(EventId.AfterHangarInit, OnHangarInit);
    }
    ...
    private bool LoadServerOptions (JsonPrefs options)
    {
        ...
        LoadConsumableInfos();
        LoadConsumableKitInfos(options);
        #endregion
        #region DailyBonus Section (Strongly After Consumable Loaded)
        if (options.Contains("DailyBattleBonus"))
        {
            var bonuses = options.ValueObjectList("DailyBattleBonus/days");
            dailyBonusInfos = new List<DailyBonusInfo>();
            foreach (var o in bonuses)
                dailyBonusInfos.Add(new DailyBonusInfo((Dictionary<string, object>)o));
        }
        #endregion
        #region WheelOfFortune Section (Strongly After Consumable Loaded)
        if (options.Contains("WheelOfFortune"))
        {
            var sectors = options.ValueObjectList("WheelOfFortune/sectors");
            roulettItems = new List<RoulettItemInfo>();
            foreach (var o in sectors)
                roulettItems.Add(new RoulettItemInfo((Dictionary<string, object>)o));

            if(options.Contains("WheelOfFortune/attempts"))
                roulettAttemptsCount = options.ValueInt("WheelOfFortune/attempts");
        }
        #endregion
        ...
        if (options.Contains ("PhotonSettings")) {
            ...
            AllowBackgroundConnection = options.ValueBool ("allowBackgroundConnection", false);
            if (AllowBackgroundConnection) {
                PhotonNetwork.BackgroundTimeout = options.ValueFloat ("backgroundTimeout", 60f);
            }
    ...
    private void LoadConsumableKitInfos(JsonPrefs options)
    {
        if (!vehiclesDataStorage.ContainsKey("kits"))
        {
            Debug.LogError("No consumableKits info presented in data received from server!");
            return;
        }

        List<object> consKitsList = vehiclesDataStorage["kits"] as List<object>;
        vehiclesDataStorage.Remove("kits");
        ...
        consumableKitInfos = new Dictionary<int, ConsumableKitInfo>(consKitsList.Count);
        ...
        foreach (Dictionary<string, object> consKitsDict in consKitsList)
        {
            ...
            consumableKitInfos.Add(newConsumableKit.id, newConsumableKit);
        }

        //Грузим акции
        consumableKitOffersList = options.Contains("DiscountOfferItems/KitOffers") ? options.ValueObjectList("DiscountOfferItems/KitOffers") : null;

        if(consumableKitOffersList != null)
        {
            foreach (var t in consumableKitOffersList)
            {
                var prefs = new JsonPrefs(t);
                var id = prefs.ValueInt("id", -1);

                if(consumableKitInfos.ContainsKey(id))
                    consumableKitInfos[id].discount = new XDevs.DiscountInfo(prefs);
            }
        }
    }
...
#region Global time calculation and timer
    ...
    public static Camera CurSceneGuiCamera
    {
        get
        {
            if (HangarController.Instance && HangarController.Instance.GuiCamera)
                return HangarController.Instance.GuiCamera;
            if (BattleGUI.Instance && BattleGUI.Instance.GuiCamera != null)
                return BattleGUI.Instance.GuiCamera;
            if (Loading.Instance && Loading.Instance.GuiCamera)
                return Loading.Instance.GuiCamera;
            return null;
        }
    }
    ...
    public static tk2dCamera CurSceneTk2dGuiCamera
    {
        get
        {
            ...
            if (Loading.Instance && Loading.Instance.GuiCamera)
                return Loading.Instance.Tk2dGuiCamera;
            return null;
        }
    }
