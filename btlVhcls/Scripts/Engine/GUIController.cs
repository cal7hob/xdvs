...
public class GUIController : MonoBehaviour
{
    ...
    public static T CheckReferentObject<T>(Transform parentTransform, string path, T field, bool verbose = true)
    {
        if (field != null)
            return field;
        ...
        T resultType = targetTransform.gameObject.GetComponent<T>();

        if (resultType == null)
            DT.LogError("Cannot find component \"{0}\" in \"{1}\"!", typeof(T).ToString(), path);
