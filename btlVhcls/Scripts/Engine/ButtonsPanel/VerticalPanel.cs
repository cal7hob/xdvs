...
namespace XDevs.ButtonsPanel
{
    ...
    public class VerticalPanel : MonoBehaviour
    {
        ...
        {
            List<PanelButton> activeItems = new List<PanelButton>();
            for (int i = 0; i < buttons.Count; i++)
                if (buttons[i] != null && buttons[i].isActiveAndEnabled)
                    activeItems.Add(buttons[i]);

            for (int i = 0; i < activeItems.Count; i++)
            {
                float koef = (float)i - (((float)activeItems.Count - 1f) / 2f);
                activeItems[i].transform.localPosition = new Vector3(
                    activeItems[i].transform.localPosition.x,
                    -startYPos - (activeItems[i].height + spaceBetweenButtons) * koef,
                    activeItems[i].transform.localPosition.z);
            }
        }
