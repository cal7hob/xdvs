...
public class GoogleAnalyticsDispatcher : MonoBehaviour // TODO: перенести потом все GoogleAnalyticsWrapper.LogEvent() сюда. 
{
    ...
    void Awake()
    {
        ...
        Dispatcher.Subscribe(EventId.DailyBonusTaken, OnDailyBonusTaken);
        Dispatcher.Subscribe(EventId.FortuneWheelRolled, OnFortuneWheelRolled);
        Dispatcher.Subscribe(EventId.ConsumableKitBought, OnConsumableKitBought);
        Dispatcher.Subscribe(EventId.VideoStartRecording, OnVideoStartRecording);
        Dispatcher.Subscribe(EventId.VideoStartBroadcasting, OnVideoStartBroadcasting);
        Dispatcher.Subscribe(EventId.VideoUploadCompleted, OnVideoUploadCompleted);
    ...
    void OnDestroy()
    {
        ...
        Dispatcher.Unsubscribe(EventId.DailyBonusTaken, OnDailyBonusTaken);
        Dispatcher.Unsubscribe(EventId.FortuneWheelRolled, OnFortuneWheelRolled);
        Dispatcher.Unsubscribe(EventId.ConsumableKitBought, OnConsumableKitBought);
        Dispatcher.Unsubscribe(EventId.VideoStartRecording, OnVideoStartRecording);
        Dispatcher.Unsubscribe(EventId.VideoStartBroadcasting, OnVideoStartBroadcasting);
        Dispatcher.Unsubscribe(EventId.VideoUploadCompleted, OnVideoUploadCompleted);
    ...
    private void OnDailyBonusTaken(EventId id, EventInfo ei)
    {
        EventInfo_I info = (EventInfo_I)ei;
        int day = info.int1;
        ...
        GoogleAnalyticsWrapper.LogEvent(
           new CustomEventHitBuilder()
               .SetParameter(Category.DailyBonus)
               .SetParameter(Action.Taken)
               .SetParameter<Label>()
               .SetSubject(Subject.Day, day)
               .SetValue(ProfileInfo.Level));
    }

    private void OnFortuneWheelRolled(EventId id, EventInfo ei)
    {
        EventInfo_I info = (EventInfo_I)ei;
        EntityTypes entityType = (EntityTypes)info.int1;

        GoogleAnalyticsWrapper.LogEvent(
           new CustomEventHitBuilder()
               .SetParameter(Category.FortuneWheel)
               .SetParameter(Action.Rolled)
               .SetParameter<Label>()
               .SetSubject(Subject.Reward, entityType)
               .SetValue(ProfileInfo.Level));
    }

    private void OnConsumableKitBought(EventId id, EventInfo ei)
    {
        EventInfo_I info = (EventInfo_I)ei;
        int kitId = info.int1;
        ...
        GoogleAnalyticsWrapper.LogEvent(
            new CustomEventHitBuilder()
                .SetParameter(Category.ConsumableKitBuying)
                .SetParameter<Action>()
                .SetSubject(Subject.KitID, kitId)
                .SetParameter<Label>()
                .SetSubject(Subject.PlayerLevel, ProfileInfo.Level));
    }

    private void OnVideoStartRecording(EventId id, EventInfo ei)
    {
        GoogleAnalyticsWrapper.LogEvent(
            new CustomEventHitBuilder()
                .SetParameter(Category.EveryplayVideo)
                .SetParameter(Action.StartRecording)
                .SetValue(ProfileInfo.Level));
    }

    private void OnVideoStartBroadcasting(EventId id, EventInfo ei)
    {
        GoogleAnalyticsWrapper.LogEvent(
            new CustomEventHitBuilder()
                .SetParameter(Category.EveryplayVideo)
                .SetParameter(Action.StartBroadcasting)
                .SetValue(ProfileInfo.Level));
    }

    private void OnVideoUploadCompleted(EventId id, EventInfo ei)
    {
        GoogleAnalyticsWrapper.LogEvent(
            new CustomEventHitBuilder()
                .SetParameter(Category.EveryplayVideo)
                .SetParameter(Action.Uploaded)
                .SetValue(ProfileInfo.Level));
    }
