...
public class AppsFlyerImpl : MonoBehaviour
{
    ...
    void Awake()
    {
        if (Instance == null)
        {
            ...
            {
                Debug.LogFormat("AppsFlyerImpl. Awake(). Setting dev key {0} and appId = {1}", APPSFLYER_DEVKEY, appId);
...
#elif UNITY_WSA
            AppsFlyer.setAppsFlyerKey(APPSFLYER_DEVKEY); // По-идее только для iOS
            AppsFlyer.setAppID(appId);
#endif
    ...
    public static void TrackRichEvent(string eventName, Dictionary<string, string> eventValues)
    {
        ...
        if (Instance == null)
        {
            Debug.LogErrorFormat("AppsFlyerImpl.TrackRichEvent(). Instance == NULL!!! The event {0} cant be sent.", eventName);
            return;
        }
    ...
    static AndroidJavaClass m_clsAppsFlyer = null;
    private static AndroidJavaClass cls_AppsFlyer
    {
        get
        {
            if (m_clsAppsFlyer == null)
                m_clsAppsFlyer = new AndroidJavaClass("com.appsflyer.AppsFlyerLib");
            return m_clsAppsFlyer;
        }
    }

    void AppPaused(bool pauseStatus)
    {
        using (AndroidJavaClass cls_UnityPlayer = new AndroidJavaClass("com.unity3d.player.UnityPlayer"))
        {
            using (AndroidJavaObject cls_Activity = cls_UnityPlayer.GetStatic<AndroidJavaObject>("currentActivity"))
            {
                if (pauseStatus)
                {
                    cls_AppsFlyer.CallStatic("onActivityPause", cls_Activity);
                }
                else
                {
                    cls_AppsFlyer.CallStatic("onActivityResume", cls_Activity);
                }
            }
        }
    }
