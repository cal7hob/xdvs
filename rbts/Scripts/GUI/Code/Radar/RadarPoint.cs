...
public class RadarPoint : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        Messenger.Subscribe(EventId.MainTankAppeared, OnMainTankExists);
    }

    void OnDestroy()
    {
        Messenger.Unsubscribe(EventId.MainTankAppeared, OnMainTankExists);
    }
    ...
    private void Redraw()
    {
        if (GameData.IsGame(Game.BattleOfWarplanes | Game.BattleOfHelicopters))
        {
            RedrawForBOW();
            return;
        }
