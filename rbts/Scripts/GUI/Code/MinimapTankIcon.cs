...
public class MinimapTankIcon : MonoBehaviour 
{
    ...
    void Update () 
    {
        ...
        pxCoordsX = -1 * Mathf.Abs(Minimap.realMapCornerTR.transform.position.x - player.transform.position.x) * Minimap.scaleFactorX;//Координаты умножаем на -1, потому что отсчет от правого верхнего угла
        pxCoordsY = -1 * Mathf.Abs(Minimap.realMapCornerTR.transform.position.z - player.transform.position.z) * Minimap.scaleFactorY;
        image.transform.localPosition = new Vector3(pxCoordsX, pxCoordsY, image.transform.localPosition.z);
