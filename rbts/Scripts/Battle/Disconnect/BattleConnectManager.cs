...
namespace Disconnect
{
    public class BattleConnectManager : MonoBehaviour
    {
        ...
        public VehicleData MyLastVehicleData { get; private set; }
        ...
        void OnMasterClientSwitched(PhotonPlayer newMaster)
        {
            if (newMaster.Equals(PhotonNetwork.player))
                Messenger.Send(EventId.NowImMaster, new EventInfo_B(false));
        }

        void OnReceivedRoomListUpdate()
        {
            Messenger.Send(EventId.PhotonRoomListReceived, new EventInfo_SimpleEvent());
        }

        public void OnJoinedRoom()
        {
            state = CatcherState.InRoom;
            Messenger.Send(EventId.PhotonJoinedRoom, new EventInfo_SimpleEvent());
        }
        ...
        void OnPhotonPlayerConnected(PhotonPlayer player)
        {
            ...
            Messenger.Send(EventId.NewPlayerConnected, new EventInfo_I(player.ID));
        }
        ...
        void OnPhotonPlayerDisconnected(PhotonPlayer other)
        {
            ...
            Messenger.Send(EventId.PlayerDisconnected, new EventInfo_I(other.ID));
        }
        ...
        void OnDisconnectedFromPhoton() /*Catch own disconnect*/
        {
            if (!IsEnabled || state == CatcherState.ForcedDisconnect || playersCount < 2)
            {
                if (!BattleController.Instance.NormalDisconnect)
                {
                    StopAllCoroutines();
                    CancelInvoke();
                    Messenger.Send(EventId.PhotonDisconnectWithCause, new EventInfo_S("Disconnect due to communication problems"));
                    Messenger.Send(EventId.TroubleDisconnect, new EventInfo_SimpleEvent());
                }
                return;
            }
            ...
            if (state == CatcherState.InBattle && BattleController.MyVehicle != null)
            {
                ...
                MyLastVehicleData = BattleController.MyVehicle.data;
                MyLastPlayerStat = BattleController.MyVehicle.Statistics;

                BeforeReconnect();
                Messenger.Send(EventId.BeforeReconnecting, new EventInfo_SimpleEvent());
        ...
        void OnCreatedRoom()
        {
            Messenger.Send(EventId.NowImMaster, new EventInfo_B(true));
        ...
        void OnLeftRoom()
        {
            if (state == CatcherState.ForcedDisconnect)
                Messenger.Send(EventId.LeftRoom, new EventInfo_SimpleEvent());
        }
        ...
        {
            if (FirstConnect)
                Messenger.Send(EventId.JoinRoomFailed, new EventInfo_SimpleEvent());
            else
        ...
        public static void ClearPhotonCaches()
        {
            if (PhotonNetwork.SendMonoMessageTargets != null)
            {
            ...
            if (PhotonNetwork.PrefabCache != null)
            {
        ...
        private void Subscribes()
        {
            ...
            Messenger.Subscribe(EventId.NormalDisconnectNotice, OnDisconnectRequest);
            Messenger.Subscribe(EventId.MasterDisconnectAnswer, OnMasterDisconnectResponse);
        }
        ...
        private void Unsubscribes()
        {
            ...
            Messenger.Unsubscribe(EventId.NormalDisconnectNotice, OnDisconnectRequest);
            Messenger.Unsubscribe(EventId.MasterDisconnectAnswer, OnMasterDisconnectResponse);
        }
        ...
        private IEnumerator ForcedDisconnecting(Action afterDisconnectCallback)
        {
            if (!PhotonNetwork.isMasterClient || PhotonNetwork.otherPlayers.Length == 0)
                Messenger.Send(EventId.NormalDisconnectNotice, new EventInfo_I(BattleController.MyPlayerId),
                    Messenger.EventTargetType.ToMaster);
            else
                Messenger.Send(EventId.NormalDisconnectNotice, new EventInfo_I(BattleController.MyPlayerId),
                    Messenger.EventTargetType.ToSpecific, GetNextMasterId());
        ...
        private void CatchOthersDisconnect(VehicleData data)
        {
        ...
        private void OnTankConnected(EventId id, EventInfo ei)
        {
            ...
            VehicleData data = BattleController.allVehicles[info.int1].data;
        ...
        private IEnumerator CheckBusyList()
        {
            ...
            while (PhotonNetwork.inRoom)
            {
                ...
                if (removedCount > 0)
                {
                    ...
                    Messenger.Send(EventId.RoomBusyListTrimmed, new EventInfo_SimpleEvent());
                }
                yield return wait;
            }
        }
        ...
        {
            MessageBox.HideHardMessage();
            MessageBox.Show(MessageBox.Type.Info, Localizer.GetText("NTPConnectionError"));
            ...
            Messenger.Send(EventId.PhotonDisconnectWithCause, new EventInfo_S(disconnectCause));
        }
        ...
        private void BeforeReconnect()
        {
            ...
            else
            {
                ...
                MyLastVehicleData.armor = MyLastVehicleData.maxArmor;
            }
        ...
        private void OnDisconnectRequest(EventId id, EventInfo ei)
        {
            ...
            Messenger.Send(EventId.MasterDisconnectAnswer, new EventInfo_SimpleEvent(), Messenger.EventTargetType.ToSpecific, playerId);
        }
