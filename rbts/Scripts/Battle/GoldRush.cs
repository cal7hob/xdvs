...
public class GoldRush
{
	...
	public static void Disactivate()
	{
		...
		Messenger.Unsubscribe(EventId.GoldRushPlayerStakes, OnPlayerStakes);
	...
	public static void ActivateForBattle()
	{
		...
		activated = true;
		Messenger.Subscribe(EventId.GoldRushPlayerStakes, OnPlayerStakes);
	...
	public static void PlayerStake(int stake)
	{
		if (!activated)
			return;
		Messenger.Send(EventId.GoldRushPlayerStakes, new EventInfo_I(stake), Messenger.EventTargetType.ToMaster);
	}
	...
	private static void OnVehicleOutOfTime(EventId id, EventInfo ei)
	{
		...
		if (vehicleId == BattleController.MyPlayerId && instance.leader == BattleController.MyPlayerId && instance.awardPermission)
		{
			...
			Messenger.Send(EventId.GoldAcquired, new EventInfo_I(instance.currentStake));
	...
	{
		if (!PhotonNetwork.isMasterClient || BattleController.allVehicles.Count < 2)
			return;
