...
public class EffectPoolDispatcher : MonoBehaviour
{
	...
	public class Effect
	{
		...
		{
			transform = Instantiate(effectObject, -Vector3.up * 10000, Quaternion.identity).transform;
	...
	public static GameObject GetFromPool(GameObject _effect, Vector3 _position, Quaternion _rotation, bool useEffectMover = false, Transform moverTarget = null)
	{
		...
		if (useEffectMover)
		{
			...
			if (!mover)
			{
				Debug.LogErrorFormat(effect, "There is no EffectMover component attached to effect ({0}). Added. Please fix effect prefab.", effect.name);
				mover = effect.AddComponent<EffectMover>();
			}
			mover.Target = moverTarget;
		}
