...
#pragma warning restore 67
        ...
        /*{
            ...
            {"xdevs.vip_kit", new SocialPrice(0.99f, SocialCurrency.Dollar) },
            {"xdevs.gold_kit", new SocialPrice(0.99f, SocialCurrency.Dollar) },
        ...
        {
            if (prices == null)
                prices = new Dictionary<string, SocialPrice>();

            if (prices.Count == 0)
            {
                foreach (var socialPrice in GameData.socialPrices)
                {
                    if (socialPrice.Key != null && socialPrice.Value != null)
                    {
                        float price = Convert.ToSingle(socialPrice.Value,
            ...
            return prices.ContainsKey(item_id) ? prices[item_id] : null;
        }

        public string GetPriceStringById (string item_id)
        {
            return prices.ContainsKey(item_id) ? prices[item_id].PriceString : null;
        }
        ...
        void OnInitComplete ()
        {
            ...
            {
                Messenger.Send(EventId.AfterWebPlatformDefined, null);
            }
#else
            ...
            if (FB.IsLoggedIn)
            {
                FB.API("/me?fields=first_name,last_name", HttpMethod.GET, graphResult =>
                {
                    if (string.IsNullOrEmpty(graphResult.Error))
                    {
                        ...
                        {
                            Messenger.Subscribe(EventId.ServerDataReceived, CheckAppPage);
                            LoginSucceed(this);
                        }
#else
                        LoginSucceed(this);
