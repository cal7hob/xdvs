...
namespace SocialNetworks
{
    ...
    public class Odnoklassniki : OdnoklassnikiWebplayerAPI, ISocialService
    {
        ...
        private string AppLink
        {
            get
            {
                return string.Format("https://ok.ru/game/{0}",
                    ServiceSettings.Services[ServiceSettingsKeys.Service.Odnoklassniki][
                        ServiceSettingsKeys.Field.AppShortname]);
            }
        }
...
#pragma warning restore 67
        ...
        public void PostAchievement(AchievementsIds.Id webAchievement, string text)
        {
            ...
            var postText = string.Format("{0} {1}", text, AppLink);

            Application.ExternalCall("OKAPIWrapper.achievement_report", webAchievement, postText);
        }
        ...
        public void Post(string text, Texture2D img)
        {
            ...
            StartCoroutine(UploadImage(img, delegate (string token)
            {
                ...
                Application.ExternalCall("OKAPIWrapper.post", postText, token);
            }, LogToConsole));
        }
        ...
        public void ShowPayment(string item_id)
        {
            ...
            JSshowPayment(filteredItem.name, "", filteredItem.id, (int)filteredItem.price.Value, null, null, "ok", "true");
            XdevsSplashScreen.SetActiveWaitingIndicator(false);
        }
                    ...
                    if (fResult)
                    {
                        ...
                        GoogleAnalyticsWrapper.LogEvent(
                            new CustomEventHitBuilder()
                                ...
                                .SetParameter(GAEvent.Action.GotViaOdnoklassnikiInvitation)
