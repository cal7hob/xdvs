...
public class XdevsSplashScreen : MonoBehaviour
{
    ...
    private void Awake()//Если обнулять instance в OnDestroy() то будет ошибка перетирания instance. Тогда уж лучше инстанировать SplashScreen на сцене лоадинг
    {
        ...
        lblSplashAdvice.color = GameData.IsGame(Game.FutureTanks) ? new Color(1, 0.74f, 0.26f, 1) : Color.white;
    }
    ...
    public static void SetActive(bool en, bool showLabels = false)
    {
        if (Instance == null)
        {
            Debug.LogError("SplashScreen.Instance == null!");
            return;
        }
        ...
        if (en)
        {
            var path = Path.Combine(
                ...
                Instance.splashscreens.GetRandomItem());

            Instance.splash.texture = Resources.Load(path) as Texture2D;

            if (Instance.splash.texture == null)
            {
                Debug.LogError("Splash texture is null!");
                return;
            }
