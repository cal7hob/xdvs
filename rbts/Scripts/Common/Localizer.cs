...
public class Localizer : MonoBehaviour
{
    ...
    {
        string result = GetTextSilently(key, parameters);
        if (string.IsNullOrEmpty(result))
        {
            Debug.LogErrorFormat("Localization: key '{0}' not found.", key);
        }

        return result;
    }

    public static string GetTextSilently(string key, params object[] parameters)
    {
        if (!loaded)
        {
        ...
        key = key.ToUpper();
        string result;
        if (key == null || !locDictionary.TryGetValue(key, out result))
            return "";
    ...
    private IEnumerator LoadDictionary()
    {
        ...
        Messenger.Send(EventId.OnLanguageChange, new EventInfo_SimpleEvent());
    }
            ...
            try
            {
                ...
                if (instance.language == langForLoad)
                {
                    ...
                    foreach (var kvp in data)
                    {
                        ...
                        locDictionary[kvp.Key.ToUpper()] = val;
                    }
                }
                else
                {
