...
public class PushwooshRequestManager : MonoBehaviour
{
    ...
    public IEnumerator CreateMessage(List<object> notifications)
    {
        ...
        yield return StartCoroutine(CreateMessageApiRequest(json));
    }
    ...
    public IEnumerator SetTags(string hwid)
    {
        ...
        yield return StartCoroutine(SetTagsApiRequest(json));
    }

    private IEnumerator SetTagsApiRequest(string json)
    {
        ...
        Debug.Log("pushwoosh setTags error: " + request.error);
