...
public class GoToBattleTutorial : Tutorial
{
    ...
    protected override IEnumerator RefreshingRoutine()
    {
        ...
        Messenger.Send(EventId.ChangeElementStateRequest, new EventInfo_U(new ChangeElementStateRequestInfo(this, new HashSet<System.Type>() { typeof(ScoresController), typeof(RightPanel) }, !IsActive)));
        ...
        if (IsActive)
        {
            ...
            {
                didPlayVoice = true;
                Messenger.Send(EventId.VoiceRequired, new EventInfo_I((int)VoiceEventKey.GoToBattleLesson));
            }
        }
    }
}
