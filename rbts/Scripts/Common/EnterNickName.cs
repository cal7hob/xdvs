...
public class EnterNickName : HangarPage {
    ...
    protected override void ProfileChanged () {
        ...
        if (GUIPager.ActivePage == "EnterName" && ProfileInfo.nickEntered) {
    ...
    public void PopUpNicknameWindow(EventId id = 0, EventInfo info = null)
    {
        ...
        GUIPager.SetActivePage("EnterName", true, true, (int)VoiceEventKey.EnterNameLesson);
    }
    ...
    private void Submit()
    {
        ...
        {
            GUIPager.SetActivePage("MainMenu");
            Messenger.Send(EventId.NickNameManuallyChanged, new EventInfo_SimpleEvent());
        });
    }
