...
public class PatternOffer : ShopOffer
{
    ...
    {
        base.Awake ();
        Messenger.Subscribe(EventId.OnLanguageChange, SetInfo);
    }

    public override void Initialize(int id, int discount, double endTime)
    {
        base.Initialize(id, discount, endTime);
        Messenger.Subscribe(EventId.PatternShopFilled, UpdateSaleSticker);
    }

    protected override void OnDestroy()
    {
        base.OnDestroy ();
        Messenger.Unsubscribe(EventId.PatternShopFilled, UpdateSaleSticker);
