...
public abstract class ShopItemCell : MonoBehaviour
{
    ...
    public virtual void Set<TShopItemCell>(IShopItem shopItem, bool isLastItem)
        where TShopItemCell : ShopItemCell
    {
        ...
        SetScrollableArea(shopWindow.ScrollableArea, isLastItem);
    ...
    private void SetScrollableArea(tk2dUIScrollableArea scrollableArea, bool isLastItem)
    {
        var cellOffsetY
        ...
        float h = (layout.GetMaxBounds() - layout.GetMinBounds()).y;

        layout.transform.parent = scrollableArea.contentContainer.transform;
        layout.transform.localPosition = new Vector3(cellOffsetX - w / 2, cellOffsetY + h / 2, -1);

        cellOffsetX += w + (isLastItem ? 0 : CELL_SPACING);

        scrollableArea.ContentLength = cellOffsetX;
    }
}
