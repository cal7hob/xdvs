...
public class RefillGasTank : MonoBehaviour
{
    ...
    void OnEnable () {
        ...
        Messenger.Subscribe (EventId.FriendInviteSuccess, FriendInviteSuccess);
        HangarController.OnTimerTick += UpdateFuel;
    }

    void OnDisable () {
		SocialSettings.OnLastInviteChanged -= LastFriendInviteChanged;
        Messenger.Unsubscribe (EventId.FriendInviteSuccess, FriendInviteSuccess);
        HangarController.OnTimerTick -= UpdateFuel;
    }
    ...
    public void ShowRefillGasTankWindow()
    {
        ...
        {
#endif
        GUIPager.SetActivePage("RefillGasTank", true, !GameData.IsGame(Game.Armada));
