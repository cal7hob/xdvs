...
public class ScoresController : MonoBehaviour {
    ...
    [Header("Rotate One Sprite...")]
	public Transform btnToggleArrow;
    [Header("or toggle another two")]
    public GameObject arrow_ActiveWhenOpened;
    public GameObject arrow_ActiveWhenClosed;
    ...
    private void Awake()
    {
        ...
        Messenger.Subscribe(EventId.AfterHangarInit, AfterHangarInitHandler);
        Messenger.Subscribe(EventId.MessageBoxChangeVisibility, OnMessageBoxChangeVisibility);
        Messenger.Subscribe(EventId.ChangeElementStateRequest, OnChangeElementStateRequest);
    }
    ...
    private void OnDestroy()
    {
        ...
        Messenger.Unsubscribe(EventId.AfterHangarInit, AfterHangarInitHandler);
        Messenger.Unsubscribe(EventId.MessageBoxChangeVisibility, OnMessageBoxChangeVisibility);
        Messenger.Unsubscribe(EventId.ChangeElementStateRequest, OnChangeElementStateRequest);
    ...
    public IEnumerator ScoresReceived(JSONObject data)
    {
        ...
        Messenger.Send(EventId.ScoresHighlightedItemsReady, null);
        ...
        UpdateVisibility();
        Messenger.Send(EventId.ScoresBoxActivated, null);
    }
    ...
    void Start()
    {
        ...
        if (HangarController.FirstEnter && ProfileInfo.Level <= GameData.hideScoresTillLevel && !PlayerPrefs.HasKey(REGKEY_SCORES_BOX_STATE))
            SetActive(false);
        else
            SetActive(!PlayerPrefs.HasKey(REGKEY_SCORES_BOX_STATE) || Convert.ToBoolean(PlayerPrefs.GetInt(REGKEY_SCORES_BOX_STATE)));
    }
    ...
    void VerticalAlign()
    {
        ...
        if (overrideBottomPanelHeight != default(float))
            ...
            layoutBottomPosition = HangarController.Instance.bottomGuiPanel.GetComponent<Renderer>().bounds.size.y;
	...
	{
		SetActive(!mainLayout.gameObject.GetActive());
	}
	...
	public void SetActive(bool en)
	{
        ...
        if (ItemRotationController.Instance != null)
            ItemRotationController.Instance.SetTouchableArea(0, null);
        if(btnToggleArrow)
		    btnToggleArrow.localRotation = Quaternion.Euler(0, 0, en ? 180 : 0);
        else if(arrow_ActiveWhenOpened || arrow_ActiveWhenClosed)
        {
            if (arrow_ActiveWhenOpened)
                arrow_ActiveWhenOpened.SetActive(en);
            if (arrow_ActiveWhenClosed)
                arrow_ActiveWhenClosed.SetActive(!en);
        }
		PlayerPrefs.SetInt(REGKEY_SCORES_BOX_STATE, Convert.ToByte(en));
	}
