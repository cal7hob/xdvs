...
//		Debug.Log ("FacebookManager.show()");
    ...
    public static void FriendsInviteRequest () {
        ...
        FB.Mobile.AppInvite(
            appLinkUrl: new Uri(
                ServiceSettings.Services[ServiceSettingsKeys.Service.Facebook][ServiceSettingsKeys.Field.AppLinkURL]),
            previewImageUrl: null,
            callback: s_instance.AppInviteCallback);
#else
        FB.AppRequest(Localizer.GetText("textVkInvitation"), null, null, null, null, null, null, s_instance.FriendsInviteCallback);
#endif
    }
        ...
        FB.API("me/photos", HttpMethod.POST, delegate(IGraphResult result)
        {
            ...
            if (string.IsNullOrEmpty(result.Error))
            {
                FB.API("/" + result.ResultDictionary["id"] + "?fields=source", HttpMethod.GET, delegate(IGraphResult graphResult)
                {
                    ...
                    {
                        // Здесь, возможно, правильнее использовать App Link URL, т.к. он будет хорошо работать
                        // на разных платформах
                        FB.ShareLink(
                            new Uri("https://apps.facebook.com/" + ServiceSettings.Services[ServiceSettingsKeys.Service.Facebook][ServiceSettingsKeys.Field.Namespace]),
    ...
    void Awake () {
        Messenger.Subscribe (EventId.AfterHangarInit, StartInit, 1);
#if UNITY_WEBPLAYER || UNITY_WEBGL
        Messenger.Subscribe (EventId.AfterWebPlatformDefined, WebPlatformDefined, 1);
#endif
        s_instance = this;
    }
...
#if UNITY_WEBPLAYER || UNITY_WEBGL
        Messenger.Unsubscribe (EventId.AfterWebPlatformDefined, WebPlatformDefined);
#endif
    }
    ...
    void Start () {
        ...
        {
            loginButton.lFbRewardMoney.text = ProfileInfo.SocialActivationReward.value.ToString ("N0", GameData.instance.cultureInfo.NumberFormat);
        ...
        Messenger.Send(EventId.AfterFacebookInitialized, new EventInfo_SimpleEvent());
        return;
...
//        friendsList.SetActive (false);
    ...
    private void OnInitComplete()
    {
        ...
        if (!m_hangarInitialized) {
            Messenger.Send (EventId.AfterFacebookInitialized, null);
        }
...
//        Debug.Log("Is game showing? " + isGameShown);
    ...
    void OnInfoLoaded (IGraphResult result) {
        ...
        Messenger.Send(EventId.AfterFacebookMainInfoLoaded, null);
        SendScores();
    }
