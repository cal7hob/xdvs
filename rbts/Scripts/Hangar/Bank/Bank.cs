...
public class Bank : MonoBehaviour
{
    ...
    [SerializeField] private GameObject tapJoyOfferPrefab;
    [SerializeField] private GameObject freeTankOfferPrefab;

    [Header("Frame prefab settings:")]
    [SerializeField] private BankLot bankLotPrefab;
    [SerializeField] private float framesPosY;
    [SerializeField] private Vector2 frameDimensions;
    [SerializeField] private float framesOffset;

    [Header("Kits")]
    [SerializeField] private tk2dTextMesh[] vipKitPriceLabels;
    [SerializeField] private tk2dTextMesh[] goldKitPriceLabels;
    [SerializeField] private tk2dTextMesh[] vehicleKitPriceLabels;
    [SerializeField] private tk2dTextMesh lblOneTimeOfferVIP30DaysDetails;
    [SerializeField] private tk2dTextMesh lblOneTimeOfferGoldSubscriptionDetails;
    [SerializeField] private tk2dTextMesh lblOneTimeOfferGoldSubscriptionDetails1;
    [SerializeField] private tk2dTextMesh lblOneTimeOfferGoldSubscriptionDetails2;
    [SerializeField] private tk2dTextMesh lblOneTimeOfferGoldSubscriptionDetailsAwardAmount;

    [SerializeField] private tk2dTextMesh lblOneTimeOfferStarterPackDetails;
    [SerializeField] private tk2dTextMesh vehicleKitDetails_Vehicle;
    [SerializeField] private tk2dBaseSprite vehicleKitDetails_PatternImage;
    [SerializeField] private tk2dTextMesh vehicleKitDetails_PatternStats;
    [SerializeField] private tk2dBaseSprite vehicleKitDetails_DecalImage;
    [SerializeField] private tk2dTextMesh vehicleKitDetails_DecalStats;
    ...
    void Awake()
    {
        Messenger.Subscribe(EventId.AfterHangarInit, BankInit);
        Messenger.Subscribe(EventId.AfterHangarInit, SetAdsFreeText);
        Messenger.Subscribe(EventId.OnLanguageChange, OnLanguageChange);
        Messenger.Subscribe(EventId.ProfileInfoLoadedFromServer, OnLanguageChange);
        Messenger.Subscribe(EventId.BankInitialized, OnBankInitialized);
    ...
    void OnDestroy()
    {
        Messenger.Unsubscribe(EventId.AfterHangarInit, BankInit);
        Messenger.Unsubscribe(EventId.AfterHangarInit, SetAdsFreeText);
        Messenger.Unsubscribe(EventId.OnLanguageChange, OnLanguageChange);
        Messenger.Unsubscribe(EventId.ProfileInfoLoadedFromServer, OnLanguageChange);
        Messenger.Unsubscribe(EventId.HangarTimerTick, OnHangarTimerTick);
        Messenger.Unsubscribe(EventId.BankInitialized, OnBankInitialized);
    ...
    private static void NeededBankLotToView(Transform trans, tk2dUIScrollableArea scrollableArea)
    {
    ...
    private void CreateBankFrames()
    {
        ...
        Messenger.Subscribe(EventId.HangarTimerTick, OnHangarTimerTick);
        Messenger.Send(EventId.BankInitialized, new EventInfo_SimpleEvent());
    }
    ...
    private void UpdatePriceLabels()
    {
        ...
        HelpTools.SetTextToAllLabelsInCollection(vipKitPriceLabels,
            PriceLocalizationAgent.GetLocalizedString(vipKitIdString, vipKitPriceLabels[0], "lblBuyKit"));
        lblOneTimeOfferVIP30DaysDetails.text = Localizer.GetText("lblOneTimeOfferVIP30DaysDetails",
            PriceLocalizationAgent.GetLocalizedString(vipKitIdString, lblOneTimeOfferVIP30DaysDetails, "lblBuyKit"));

        HelpTools.SetTextToAllLabelsInCollection(goldKitPriceLabels,
            PriceLocalizationAgent.GetLocalizedString("xdevs.gold_kit", goldKitPriceLabels[0], "lblBuyKit"));
        lblOneTimeOfferGoldSubscriptionDetails.text = Localizer.GetText("lblOneTimeOfferGoldSubscriptionDetails",
            PriceLocalizationAgent.GetLocalizedString("xdevs.gold_kit", lblOneTimeOfferGoldSubscriptionDetails, "lblBuyKit"));
        HelpTools.SetTextToAllLabelsInCollection(vehicleKitPriceLabels,
            PriceLocalizationAgent.GetLocalizedString("xdevs.vehicle_kit", vehicleKitPriceLabels[0], "lblBuyKit"));
        lblOneTimeOfferStarterPackDetails.text = Localizer.GetText("lblOneTimeOfferStarterPackDetails",
            PriceLocalizationAgent.GetLocalizedString("xdevs.vehicle_kit", lblOneTimeOfferStarterPackDetails, "lblBuyKit"));
    }
    ...
    private void ReсreateKitsIfNeeded()
    {
        ...
        foreach (var kitTypePair in activeKits)
            foreach(var kitPair in kitTypePair.Value)
                if(kitPair.Value != null && kitPair.Value.kitBehaviour)
                {
                    if (GUIPager.ActivePage == kitPair.Value.Page)
                        GUIPager.SetActivePage("MainMenu");
                    Destroy(kitPair.Value.kitBehaviour.gameObject);
                }
        ...
        if (GUIPager.ActivePage == "Bank" && tabsGroup != null && tabsGroup.SelectedIndex == (int)Tab.Kits && itemsCount == 0)
        {
            GUIPager.SetActivePage("MainMenu");
        }
        else if(itemsCount > 0)
        {
            ...
            foreach (var kitTypePair in activeKits)
                foreach (var kitPair in kitTypePair.Value)
                {
                    ...
                    kitPair.Value.kitBehaviour = InstantiateFrame(go, go.name, kitTransforms).GetComponent<BankKits.KitBehaviour>();
                    kitPair.Value.kitBehaviour.Init(kitPair.Value);
                }
    ...
    private void OnBankInitialized(EventId id, EventInfo info)
    {
        GUIPager.EnqueueAction(() =>
        {
            if (!HangarController.FirstEnter)
            {
                ...
                {
                    HangarController.Instance.GoToBank(Tab.Kits, false);
