...
public class VKManager : MonoBehaviour, ISocialService
{
    ...
    public void InviteFriend() { Application.ExternalCall("ShowInvite"); }
    ...
    private IEnumerator updateSocialActivity()
    {
        ...
        if(!bookmarked)
            yield return StartCoroutine(CheckPermissions(256, delegate(bool havePermissions)
            {
                if(havePermissions)
                {
                    StaticType.SocialSettings.Instance<ISocialSettings>().ReportSocialActivity(SocialAction.bookmarked);
                    bookmarked = true;
                }
            }));
        ...
        if (!invited)
            yield return StartCoroutine(GetFriendsAppUsersCount(delegate(int count)
            {
                if(count >= 3)
                {
                    StaticType.SocialSettings.Instance<ISocialSettings>().ReportSocialActivity(SocialAction.invited);
                    invited = true;
                }
            }));
    ...
    public void GiveMoney(string item_name)
    {
        ...
        if (Prices.ContainsKey(item_name))
        {
            ...
            StaticType.TapJoy.Instance<ITapJoy>().TrackPurchase(item_name, Prices[item_name].Currency.ToString(), Prices[item_name].Value);
        }
#endif
        Event(XD.Message.LockUI, false);
