...
class VkMobile : MonoBehaviour, ISocialService, ISubscriber
{
#region ISocialService implementation
    ...
    public static List<VkUser> FriendsToInvite
    {
        get
        {
    ...
    public void InviteFriend()
    {
        ...
        GUIPager.SetActivePage("VkFriendInvitation", true ,true);
    }
    ...
    {
        var attachmentText = GameData.CurrentGame.ToString();
    ...
    public SocialNetworkInfo SocialNetworkInfo
    {
        get
        {
            return new SocialNetworkInfo(SocialPlatform.Vkontakte, "ВКонтакте", "linkPanelvk");
        }
    }
...
#region ISubscriber
    ...
    public void Reaction(XD.Message message, params object[] parameters)
    {
        ...
        switch (message)
        {
            case XD.Message.PlatformLogin:
                ...
                if (typePlatform != TypeExternalPlatform.Vkontakte)
                {
                    return;
                }
#if DEBUG_TAG_TF
            ...
            case XD.Message.PlatformScreenshotPost:
                ...
                if (service != null && service.SocialNetworkInfo != null && service.SocialNetworkInfo.Platform == SocialPlatform.Vkontakte)
                {
            ...
            case XD.Message.Button:
                ...
                if (socialPlatform != null && socialPlatform == SocialPlatform.Vkontakte)
                {
                    bool set = true;
                    set = (bool)parameters.Get<bool>();
#if DEBUG_TAG_TF
                    Debug.Log("TF: VKMobile: ButtonKey.SocialLogin: " + set);
#endif
                    if (set == true)
                    {
                        //Логинимся
    ...
    private void Start()
    {
        ...
        {
 #if DEBUG_TAG_TF
            Debug.Log("TF: VKMobile.Start() 4");
 #endif
            StartCoroutine(Init());
        }
 #if DEBUG_TAG_TF
        Debug.Log("TF: VKMobile.Start() 5");
 #endif
#endif
    }
    ...
    {
        VKSdk.InitializationFinished  += delegate {
#if DEBUG_TAG_TF
            Debug.LogError("TF: VKMOBILE InitializationFinished");
#endif
            InitializationFinished(this, SocialInitializationState.Success); };
        VKSdk.LoginSucceed += delegate {
#if DEBUG_TAG_TF
            Debug.LogError("TF: VKMOBILE loginsucceed");
#endif
            StartCoroutine(Init()); };
        VKSdk.LoginFail += delegate {
#if DEBUG_TAG_TF
            Debug.LogError("TF: VKMOBILE loginfail"); 
#endif
            LoginFail();
            };
        VKSdk.LogoutSucceed += delegate {
#if DEBUG_TAG_TF
            Debug.Log("TF: VKMOBILE LogoutSucceed");
#endif
            LogoutSucceed(); };
    }
    ...
    IEnumerator GetGroupsInfo ()
    {
        ...
        Action<string> onComplete = delegate(string jsonResult)
        {
            ...
            foreach (var current in groupsObj)
            {
                ...
                var info = new SocialNetworkGroupInfo(prefs.ValueString("gid"), prefs.ValueString("name"), "vk",
...
#region test
        var list = new List<SocialNetworkGroup>();
        var info = new SocialNetworkGroupInfo("68191181", "Iron Tanks", 
                GameData.IsGame(Game.BattleOfWarplanes|Game.SpaceJet)? "socvk":"vk",
                "https://vk.com/irontanks");
        var group = new SocialNetworkGroup(info, true, this); 
        list.Add(group);
        info = new SocialNetworkGroupInfo("78616012", "Игры ВКонтакте",
                 GameData.IsGame(Game.BattleOfWarplanes | Game.SpaceJet) ? "socvk" : "vk",
    ...
    IEnumerator Init()
    {
        ...
        Player = new Tanks.Models.Player(SocialPlatform.Vkontakte, me.Id, me.FirstName, me.LastName, me.AvatarUrl);
        LoginSucceed(this);
    }
    ...
    private IEnumerator ShareScreenshot(string description)
    {
        ...
        TypeExternalPlatform typePlatform = TypeExternalPlatform.Vkontakte;
