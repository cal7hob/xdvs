...
public class Moimir : MonoBehaviour, ISocialService
{
    ...
    public void Post(string text, Texture2D img)
    {
        ...
        UploadImage(img, delegate(string img_url)
        {
            ...
            {
                {"title", GameData.IsGame(Game.IronTanks) ? "Iron Tanks" : "Future Tanks"},
	...
	public void GetFriendsAppUsers()
    {
        ...
        callByCallbackAndParams("mailru.common.friends.getAppUsers", delegate(object obj, Callback callback)
		{
            ...
            {
				#if DEBUG_FOR_WEB_GL
				Debug.LogError("TF: Moimir.GetFriendsAppUsers() -> obj as List<object> != null");
				#endif
                friends = obj as List<object>;
            }
			#if DEBUG_FOR_WEB_GL
			Debug.LogError("TF: Moimir.GetFriendsAppUsers() -> friends_loaded = true;");
			#endif
    ...
    {
        Debug.LogError("TF: PopulatePrices");
	...
	void GetProfileInfoCallback(string result)
	{
		...
		if (!profile_info.ContainsKey("first_name"))
		{
			profile_info["first_name"] = ProfileInfo.PlayerName;
		}
		if (!profile_info.ContainsKey("pic_50"))
		{
			profile_info["pic_50"] = "";
		}
	...
	{
        Debug.Log("TF: MoiMir: JSONFormat: service_id: " + item.id + "; service_name: " + item.name + "; mailiki_price: " + item.price.Value);
