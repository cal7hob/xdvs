...
public enum EventId
{
    ...
    AfterHangarInit						= 11,
    MainTankAppeared					= 12,
    //TankWeaponReloaded					= 13, // Deprecated, see WeaponReloaded
    ...
    TankEffectCancelled					= 203, // effectId, playerId
    ...
    StartBurstFire                      = 301, // playerId, shellId
    ...
    SACLOSAimed                         = 905, // Имеется ввиду наведение, а не прицеливание Aiming().
    ...
    // Применять с осторожностью и только в том случае, если не подходит ни один из вышеперечисленных EventInfo.
    FirstRequestToMaster                = 1500,
    TankTakesDamage                     = 1501, // victim, damage, attacker, shellType, position (Vector3)
    StartBuff                           = 1502,
    StopBuff                            = 1503,
    UseBuffBlocker                      = 1504, // playerID (int), blocked buffs (List<int>), action add/remove (bool/false)
