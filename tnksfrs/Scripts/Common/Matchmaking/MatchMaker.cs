    ...
    public static class MatchMaker
    {
            ...
            int group)
        {
            ...
            foreach (RoomInfo roomInfo in rooms)
            {
                ...
                if ((int)roomInfo.CustomProperties["mp"] != mapId)
                {
                    //Debug.LogErrorFormat("Room => Room map id '{0}' is not equals '{1}'", roomInfo.CustomProperties["mp"], mapId);
                    continue;
                }

                if (!roomInfo.IsOpen)
                {
                    //Debug.LogError("Room => !room.IsOpen");
                    continue;
                }

                if ((int)roomInfo.CustomProperties["gm"] != (int)GameData.Mode)
                {
                    //Debug.LogErrorFormat("Room => game mode is not equals '{0}'", GameData.Mode);
                    continue;
                }

                if (busyPlacesInRoom >= GameData.maxPlayers)
                {
                    //Debug.LogErrorFormat("Room => busyPlacesInRoom >=  '{0}'", GameData.maxPlayers);
                    continue;
                }

                if (!RoomIsOpenForMe(roomInfo))
                {
                    Debug.LogError("Room => !RoomIsOpenForMe");
                    continue;
                }
            ...
            Debug.LogErrorFormat("Try get room: group '{0}'", selectedGroup);
            if (!roomDict.ContainsKey(selectedGroup))
            {
                ...
                {
                    var mapName = Enum.Parse(typeof(XD.MapId), mapId.ToString()).ToString();
        ...
        {
            Debug.LogErrorFormat("Try create room - Group: {0}; MapId: {1}; Mode: {2}; Time: {3}", selectedGroup, mapId, GameData.Mode, PhotonNetwork.time);
        ...
        public static int GetTeamForNewPlayer(TankData playerData)
        {
        ...
        public static int[] CountTeamMembers(bool isBot = false)
        {
            int[] teamMembers = {0, 0};

            if (XD.StaticContainer.BattleController.Units == null)
            {
                return teamMembers;
            }

            foreach (var vehicle in XD.StaticContainer.BattleController.Units.Values)
            {
                if (!isBot && vehicle.IsBot)
                {
                    continue;
                }
            ...
            /*Debug.Log("Team 0: " + teamMembers[0], null);
            Debug.Log("Team 1: " + teamMembers[1], null);*/
            ...
            /*Debug.Log("Busy Team 0: " + teamMembers[0], null);
            Debug.Log("Busy Team 1: " + teamMembers[1], null);*/
        ...
        public static bool CanKickBotFromTeam(int teamId)
        {
        ...
        /// <returns>строку страны для комнаты ('**' для обычных стран)</returns>
        ...
        private static int GetTeamIdForNewPlayer(TankData playerData)
        {
            if (GameData.Mode != GameData.GameMode.Team)
                return 0;

            PlayerDisconnectInfo info = BattleConnectManager.Instance.GetDisconnectInfoForPlayer(playerData.innerId);
            if (info != null)
                return info.TeamId;
            ...
            int[] teamMembers = CountTeamMembers(BotDispatcher.IsBotId(playerData.playerId));
            int supposedTeamId = teamMembers[0] > teamMembers[1] ? 1 : 0;

            if (BotDispatcher.IsBotId(playerData.playerId))
            {
                //Debug.Log("BotDispatcher.IsBotId, Return: " + supposedTeamId);
            ...
            // Для спецстраны
            if (IfCountryIsSpecial(thisCounty))
            {
                for (int i = 0; i < countryInfos.Length; i++)
                {
                    if (countryInfos[i].DoesForCountry(thisCounty))
                    {
                        int teamId = teamMembers[i] < MaxPlayersPerTeam || CanKickBotFromTeam(i) ? i : -1;
                        Debug.Log("countryInfos[i].DoesForCountry(" + thisCounty + "), Return: " + teamId);
                        return teamId;
                    }
                }
                Debug.Log("IfCountryIsSpecial(playerData.country), Return: " + -1);
                return -1; //Ни одна команда не подходит
            }

            // Далее - для обычной страны
            if (countryInfos[supposedTeamId].IsShared)
            {
                Debug.Log("countryInfos[supposedTeamId].IsShared, Return: " + supposedTeamId);
                return supposedTeamId;
            }

            if (teamMembers[supposedTeamId] >= MaxPlayersPerTeam)
            {
                Debug.Log("teamMembers[supposedTeamId] >= MaxPlayersPerTeam, Return: " + -1);
                return -1;
            }

            if (countryInfos[supposedTeamId].DoesForCountry(thisCounty))
            {
                Debug.Log("countryInfos[supposedTeamId].DoesForCountry(thisCounty), Return: " + supposedTeamId);
                return supposedTeamId;
            }
        ...
        public static int GetMatchmakingGroup(int vehicleGroup)
        {
            ...
            if ((vehicleGroup == SANDBOX_VEHICLE_GROUP && StaticType.Profile.Instance<IProfile>().LevelCalculator.Level <= GameData.playerSandboxLevel))
                return SANDBOX_MATCHMAKING_GROUP;

            currentRule = SelectMatchmakerRule();

            int maxMatchmakingGroup =
                SafeLinq.Max(StaticContainer.MainData.MyUnits.Select(unit => (int)unit.VehicleGroup))
