...
public class FacebookManager : MonoBehaviour, ISender
{
    ...
    static public FacebookManager Instance
    {
        get
        {
            return s_instance;
    ...
    /* public static void Post(string text, Texture2D img)
     {
         ...
         s_instance.Event(XD.Message.LockUI, true);
         ...
         Debug.Log("TF: FB.API(me/photos) -->>");
         FB.API("me/photos", HttpMethod.POST, delegate (IGraphResult result)
         {
             Debug.Log("TF: FB.API(me/photos) 1");
             s_instance.Event(XD.Message.LockUI, false);
             if (string.IsNullOrEmpty(result.Error))
             {
                 Debug.Log("TF: FB.API(me/photo) call back, result: " + result);
                 Debug.Log("TF: FB.API(me/photo) call back, result.ResultDictionary[id]: " + result.ResultDictionary["id"]);

                 var formPost = new WWWForm();
                 formPost.AddField("message", "Я играю в Tank Force, чего и вам желаю!");
                 formPost.AddField("object_attachment", result.ResultDictionary["id"].ToString());
                 FB.API("me/feed?fields", HttpMethod.POST, delegate (IGraphResult graphResult)
                 {
                     if (string.IsNullOrEmpty(result.Error))
                     {
                         Debug.Log("TF: Good POST");
                     }
                     else
                     {
                         Debug.Log("TF: Bad post");
                     }
                 }, formPost);
             }
             else
             {
                 Debug.Log("TF: FB.API(me/photos) CALLBACK (graphResult.Error != ERROR): " + result.Error);
             }
         }, form);
     }*/
...
//        friendsList.SetActive (false);
    ...
    private void WebPlatformDefined(EventId id, EventInfo info)
    {
    ...
    private void OnInfoLoaded(IGraphResult result)
    {
        if (null == HangarController.Instance)
        {
            return;
