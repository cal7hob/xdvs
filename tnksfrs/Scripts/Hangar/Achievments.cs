...
public class Achievments : MonoBehaviour, ISubscriber
{
    ...
    private void ShowMyAchievments()
    {
        if(Social.localUser == null)
        {
    ...
    private void ReportAchievementWeb(string achievmentID)
    {
        ...
        try
        {
            ...
            if (id >= AchievementsIds.Id.NewbieTankMan)
            {
                ...
                StaticType.SocialSettings.Instance<ISocialSettings>().GetSocialService().PostAchievement(id, text);
            }
        }
...
#region Check For Completed Achievements
    ...
    public void Reaction(Message message, params object[] parameters)
    {
        ...
        switch (message)
        {
            /*case Message.PlatformReportLeaderboardScore:
                typePlatform = (TypeExternalPlatform)parameters.Get<TypeExternalPlatform>();
                //typePlatform = TypeExternalPlatform.Android;
                string liderboardName = (string)parameters.Get<string>();
                int liderboardScore = (int)parameters.Get<int>();
                Debug.Log("TF: PlatformReportLeaderboardScore: " + typePlatform);
                if (platforms.ContainsKey(typePlatform))
                {
                    platforms[typePlatform].ReportLeaderboardScore(liderboardName, liderboardScore);
                }
                else if (typePlatform == TypeExternalPlatform.AllPlatforms)
                {
                    foreach (Platform current in platforms.Values)
                    {
                        current.ReportLeaderboardScore(liderboardName, liderboardScore);
                    }
                }
                break;*/
            case Message.PlatformShowMyLeaderboard:
                typePlatform = (TypeExternalPlatform)parameters.Get<TypeExternalPlatform>();
                typePlatform = TypeExternalPlatform.Android;
                #if DEBUG_TAG_TF
                Debug.Log("TF: PlatformShowMyLeaderboard: " + typePlatform);
                #endif
                ShowMyLeaderboard();
                break;
            case Message.PlatformShowMyAchievments:
                typePlatform = (TypeExternalPlatform)parameters.Get<TypeExternalPlatform>();
                typePlatform = TypeExternalPlatform.Android;
#if DEBUG_TAG_TF
                Debug.Log("TF: PlatformShowMyAchievments: " + typePlatform);
#endif
                ShowMyAchievments();

                break;
            /*case Message.PlatformReportAchievement:
                typePlatform = (TypeExternalPlatform)parameters.Get<TypeExternalPlatform>();
                string achievmentID = (string)parameters.Get<string>();
                double progress = (double)parameters.Get<double>();
                Debug.Log("TF: PlatformReportAchievement: " + typePlatform);
                if (platforms.ContainsKey(typePlatform))
                {
                    platforms[typePlatform].ReportAchievement(achievmentID, progress);
                }
                else if (typePlatform == TypeExternalPlatform.AllPlatforms)
                {
                    foreach (Platform current in platforms.Values)
                    {
                        current.ReportAchievement(achievmentID, progress);
                    }
                }
                break;*/
