...
public class GoldRush
{
	...
	private static void OnVehicleOutOfTime(EventId id, EventInfo ei)
	{
        ...
        if (vehicleId == myPlayerID && instance.leader == myPlayerID && instance.awardPermission)
		{
	    ...
	    if (PhotonNetwork.isMasterClient && instance.leader == vehicleId && !instance.awardPermission)
	    {
	        BonusDispatcher.GenerateKillBonus(XD.StaticContainer.BattleController.Units[vehicleId], null);
	    }
	}
	
	private static void OnPlayerStakes(EventId id, EventInfo ei)
	{
	    if (!PhotonNetwork.isMasterClient || XD.StaticContainer.BattleController.Units.Count < 2)
	    {
	        return;
	    }
		...
		if (instance.currentStake == 0)
		{
			instance.leader = FindLeader(XD.StaticContainer.BattleController.GameStat);
	...
	private static int FindLeader(Dictionary<int, PlayerStat> statistics)
	{
        ...
        .Where(x => x.PlayerID != StaticType.BattleController.Instance<IBattleController>().MyPlayerId);
