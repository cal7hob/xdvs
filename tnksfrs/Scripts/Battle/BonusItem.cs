...
public class BonusItem : MonoBehaviour
{
	...
	void OnPhotonInstantiate(PhotonMessageInfo messageInfo)
	{
		...
		SetShadow();
		if (tankEffect != null)
			tankEffect.SetBonus(bonusType);
		MapBonus mb = GetComponent<MapBonus>();
		isMapBonus = (mb != null);

		if ((!XD.StaticContainer.BattleController.CurrentUnit || info.appearanceTime < BattleController.MyCreationTime) && !isMapBonus)
		{
			SetVisible(false);
			return;
		}
