...
{
    [SerializeField] private bool drawBotPaths = true;
        ...
        where TBotController : VehicleController
    {
        ...
        var vehicleData
            = new TankData(
                ...
                regeneration: 0,
                shield: 0,
                takenDamageRatio: 0) { playerId = NextBotId};
        ...
        var bot = PhotonNetwork.InstantiateSceneObject(
                prefabName: string.Format("{0}/{1}/{2}", GameManager.CurrentResourcesFolder, "BattleVehicles/BotVehicles", botVehicleName),
                position: Vector3.right*10000,
                rotation: Quaternion.identity,
                group: 0,
                data: new object[] {vehicleData, botBehaviour, kickAt})
                .GetComponent<TBotController>();

        bot.name = string.Format("{0}_{1}_{2} (bot)", VehicleTypes[GameData.ClearGameFlags(GameData.CurrentGame)], vehicleData.playerId, botBehaviour);
    ...
    public static void BotUnsubscribes(BotAI botAI)
    {
        ...
        Dispatcher.Unsubscribe(EventId.TankRespawned, botAI.OnBotBehaviourTankRespawned);
    }
