...
public class OdnoklassnikiWebplayerAPI : MonoBehaviour {
	protected void CallApiMethod(Dictionary<string,string> parameters, string game_object, string callback){
		Application.ExternalCall( "OKAPIWrapper.unity_api_call",  this.StringDictionaryToJSON(parameters), game_object, callback);
	}
	...
	public void JSshowPayment(string name, string description, string code, int price, string options, string attributes, string currency, string callback){
