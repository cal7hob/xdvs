...
public class HangarCameraController : MonoBehaviour
{
    ...
    [SerializeField] private Vector2 defaultRotationSpeed = Vector2.up*10;
    ...
    private void SaveCamInitialZPos()
    {
        if (Mathf.Abs(camInitialZ) < Mathf.Abs(HangarVehiclesSwitcher.HangarVehicleCamParams.MinDistanceToVehicle))
        {
            camInitialZ = -HangarVehiclesSwitcher.HangarVehicleCamParams.MinDistanceToVehicle;
        }
        else
        {
            camInitialZ = cam.transform.localPosition.z;
        }
    }
    ...
    void Awake()
    {
        ...
        Dispatcher.Subscribe(EventId.HangarVehicleGeometryLoaded, OnVehicleGeometryLoaded);
    ...
    void OnDestroy()
    {
        ...
        Dispatcher.Unsubscribe(EventId.HangarVehicleGeometryLoaded, OnVehicleGeometryLoaded);
    ...
    private void GetMouseControl()
    {
        ...
        {
            SetRotationSpeed(GetScreenRelPos(deltaMousePosition));
        }
        else
        {
            ApproximateToDefaultRotationSpeed();
        ...
        if (touchableArea.Contains(touch.position, true))
        {
            switch (Input.GetTouch(0).phase)
            {
                case TouchPhase.Moved:
                    SetRotationSpeed(GetScreenRelPos(touch.deltaPosition) * swipeSpeedQualifier);
                    break;
    ...
    private void Initialize(EventId id, EventInfo info)
    {
        ...
        nativeWidth = HangarController.Instance.Tk2dGuiCamera.nativeResolutionWidth;
        nativeHeight = HangarController.Instance.Tk2dGuiCamera.nativeResolutionHeight;
        SaveCamInitialZPos();
    ...
    private void OnVehicleGeometryLoaded(EventId id, EventInfo info)
    {
        ...
        if (Mathf.Abs(cam.transform.localPosition.z) < Mathf.Abs(maxCameraZoomZ))
        {
            CamToMaxZoom();
        }
    }

    private void OnVehicleInstalled(EventId id, EventInfo info)
    {
        SaveCamInitialZPos();
    }

    private void CamToMaxZoom()
    {
        var locPos = cam.transform.localPosition;
        locPos.z = maxCameraZoomZ;
    ...
    private void OnPageChange(string from, string to)
    {
        ...
        if (from == "Armory" && to != "MainMenu")
        {
            return;
        }

        if (from == "Settings" && to == "Armory")
        {
            return;
        }

        if (from == "Armory" || from == "Settings")
        {
            SetState(movingBackCamState);   
            return;
        }

        if (ReferenceEquals(currentCamState, moduleApproachingState) || ReferenceEquals(currentCamState, showingModuleState))
        {
            return;
        }
        ...
        if (to == "VehicleShopWindow" || to == "Settings")
        {
            if(!isOrbitChanging)
            {
                SaveCamInitialZPos();
    ...
    private Vector2 GetScreenRelPos(Vector2 deltaPos)
    {
        return new Vector2((float) nativeWidth / Screen.width * deltaPos.x, (float) nativeHeight / Screen.height * deltaPos.y);
    }

    private float GetScreenRelFingersDist(Vector2 touch0, Vector2 touch1)
    {
        return Vector2.Distance(GetScreenRelPos(touch0), GetScreenRelPos(touch1));
    }
    ...
    {
        var locPos = cam.transform.localPosition;
        locPos.z = Mathf.Clamp(-maxCameraZoomZ * (relDist - initialZoomFingersDist) * 2/nativeWidth + camBeforeZoomZPos, minCameraZoomZ, maxCameraZoomZ);
        cam.transform.localPosition = locPos;
    }
