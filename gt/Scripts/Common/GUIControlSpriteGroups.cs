...
public class GUIControlSpriteGroups : MonoBehaviour {
    ...
    [SerializeField]
    private List<tk2dBaseSprite> activatedOnMouseControlSprites = new List<tk2dBaseSprite>();
    ...
    void Awake()
    {
        var mc = (PlayerPrefs.GetInt("MouseControl", 0));
        if (mc == 1)
        {
            foreach (var spr in activatedOnMouseControlSprites)
            {
