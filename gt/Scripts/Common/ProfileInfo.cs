...
public static class ProfileInfo
{
    ...
    public static bool didTapjoyEngagementSend = false;
    ...
    public static bool IsGoldDiscountLimitReached { get; private set; }
    public static bool IsSilverDiscountLimitReached { get; private set; }
    public static bool IsVipDiscountLimitReached { get; private set; }
    ...
    public class Price
    {
        ...
        {
            {Interface.WWT2, new Dictionary<PriceCurrency, Color>()
                {
    ...
    static ProfileInfo()
    {
        IsVipDiscountLimitReached = false;
        IsGoldDiscountLimitReached = false;
        IsSilverDiscountLimitReached = false;
        accomplishedTutorials = new Dictionary<Tutorials, bool>(new TutorialsComparer());
...
//        dict["LastFriendsInvite"] = (double)lastFriendsInvite;
        ...
        dict["DidTapjoyEngagementSend"] = didTapjoyEngagementSend;
    ...
    public static bool ApplyLoadedData(Dictionary<string, object> data, long timestamp)
    {
        ...
        didTapjoyEngagementSend = prefs.ValueBool("DidTapjoyEngagementSend", didTapjoyEngagementSend);
        ...
        if (level > 3)
        {
            for (int i = 0; i < accomplishedTutorials.Count; i++)
            {
                accomplishedTutorials[((Tutorials) i)] = true;
            }
        }
...
#region Notifications
        ...
        GetAccomplishedTutorials(prefs.ValueObjectDict("CompletedTutorials"));
    ...
    public static void ApplyProfileChanges(Dictionary<string, object> data)
    {
        ...
        if (data.ContainsKey("Experience"))
        {
            ...
            haveApplyedChanges = true;
                ...
                #endregion
        #region DidTapjoyEngagementSend
        if (data.ContainsKey("DidTapjoyEngagementSend"))
        {
            didTapjoyEngagementSend = prefs.ValueBool("DidTapjoyEngagementSend", didTapjoyEngagementSend);
            Debug.LogFormat("2 {0}", didTapjoyEngagementSend);
            haveApplyedChanges = true;
        }
        #endregion
        #region Fuel
        ...
        if (data.ContainsKey("LastVisit"))
        {
            ...
            {
                MenuController.Instance.RefuelByTime(GameData.CurrentTimeStamp - lastVisit);
            }
            haveApplyedChanges = true;
        }
        #endregion
        #region IsFuelForInviteAvailable
...
#region TestData Чтобы показать DailyBonus
    ...
    private static void GetAccomplishedTutorials(Dictionary<string, object> dict = null)
    {
        if (dict == null)
        { // Got old profile.
