...
public class InterfaceModuleBase : AbstractClassForButtons, IInterfaceModule
{
    ...
    protected virtual void Awake()
    {
        ...
        for(int i = 0; i < eventsToShow.Count; i++)
        {
            ...
            {
                Dispatcher.Subscribe(eventsToShow[i], Show);
        ...
        for (int i = 0; i < eventsToHide.Count; i++)
        {
            ...
            {
                Dispatcher.Subscribe(eventsToHide[i], Hide);
    ...
    protected virtual void OnDestroy()
    {
        ...
        for (int i = 0; i < eventsToShow.Count; i++)
            Dispatcher.Unsubscribe(eventsToShow[i], Show);

        for (int i = 0; i < eventsToHide.Count; i++)
            Dispatcher.Unsubscribe(eventsToHide[i], Hide);
        #endregion
    }
