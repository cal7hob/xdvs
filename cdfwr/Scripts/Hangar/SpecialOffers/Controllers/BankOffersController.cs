...
public class BankOffersController : SpecialOffersController<BankOffer>
{
    ...
    {
        Dispatcher.Subscribe(EventId.BankInitialized, Init);
    ...
    {
        Dispatcher.Unsubscribe(EventId.BankInitialized, Init);
    ...
    protected override void SetSpecialOffersList()
    {
        ...
        {
            GameData.allOffersList.Add(goldBankOffer);
        }

        foreach (var silverBankOffer in silverBankOffers.Values)
        {
            GameData.allOffersList.Add(silverBankOffer);
        }
    }
    ...
    {
        return offers != null && offers.Count > 0 && offers.Any(offer => offer.Value.Remain > 0);
    }
