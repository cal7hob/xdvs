...
public class MapSelector : MonoBehaviour
{
    ...
    public AudioClip mapSelectionSound;
    ...
    [SerializeField]
    private GameObject playersCountWrapper;
    [SerializeField]
    private tk2dTextMesh lblPlayersCount;
    [SerializeField]
    private WaitingIndicatorBase waitingIndicator;
    ...
    void Awake()
    {
        Dispatcher.Subscribe(EventId.OnLanguageChange, SetLocker);
        playersCountWrapper.SetActive(false);
    }

    void OnDestroy()
    {
        Dispatcher.Unsubscribe(EventId.OnLanguageChange, SetLocker);
    }
    ...
    public void SetLocker(EventId id = 0, EventInfo info = null)
    {
        ...
        {
            if (alreadyCheckIfLocked)
            {
                return;
            }

            alreadyCheckIfLocked = true;
            if (disablerScript)
            {
    ...
    public void OnClick(tk2dUIItem btn)
    {
        //DT3.LogError("clicked {0}", btn.name);
        AudioDispatcher.PlayClip(mapSelectionSound);
