...
public abstract class BodyKitShop : Shop<BodykitShopItemCell>
{
    ...
    private void OnResumeRentingClick(string buttonName)
    {
        ...
        if (!IsOwned(bodykitInViewId))
        {
            ...
            MenuController.SetActionBoxType(ActionBoxType.Rent);
            return;
        }
        ...
        //HangarController.Instance.SetActionBoxType (ActionBoxType.Renting);
    ...
    protected override void FillPanel()
    {
        ...
        if (bodykitInViewId == 0 || GUIPager.ActivePage != GuiPageName)
            return;
    ...
    private void ShowBodykit(int bodykitId, bool playSound, bool showPrice)
    {
        ...
        if (playSound)
        {
            if (this is PatternShop || this is DecalShop)
            {
                MenuController.DecalSelectSound();
            }
            else
            {
    ...
    private void ShowPrice(int bodykitId)
    {
        ...
        {
            MenuController.SetActionBoxType(ActionBoxType.Rented);
        ...
        {
            MenuController.SetActionBoxType(ActionBoxType.Inaccessible);
            MenuController.Instance.InaccessibleBoxLabel.Parameter = Selectors[bodykitId].GetItem<Bodykit>().availabilityLevel.ToString();
    ...
    private bool CheckForSaleActionBoxes()
    {
        ...
        {
            MenuController.SetActionBoxType(ActionBoxType.RentSale);
