...
public class ModuleShop : Shop<ShopItemCell>
{
    ...
    {
        public Renderer bgForAligment;
        public tk2dUIItem btnItself;
        public tk2dTextMesh lblModuleLevel;
        public tk2dTextMesh lblText;
        public tk2dBaseSprite moduleIcon;
    ...
    [SerializeField]
    private float rangeBetweenIconAndArrow = 10;
    ...
    {
        if (ModuleInView != null && GUIPager.ActivePage == GuiPageName)
    ...
    private void UpdateArrowSprite(tk2dBaseSprite arrow, bool isAwaitingModule = false)
    {
        ...
        else if (isAwaitingModule)
        {
            sprUpgradeArrow.gameObject.SetActive(false);
        }
        else
        {
            arrow.SetSprite("new_upgrades");
        }
    }
    ...
    {
        Dictionary<VehicleInfo.VehicleParameter, ObscuredFloat> primaryParameters = Shop.CurrentVehicle.GetRealParameters();
        ...
        HangarController.Instance.FillParameterDelta(
           ...
           max: HangarController.Instance.ReloadMax[Shop.CurrentVehicle.Info.vehicleGroup],
           prim: primaryParameters[VehicleInfo.VehicleParameter.ReloadTime],
           sec: secondaryParameters[VehicleInfo.VehicleParameter.ReloadTime]);

        HangarController.Instance.FillParameterDelta(
            bar: MenuController.Instance.magazineBar,
            max: HangarController.Instance.MagazineMax[Shop.CurrentVehicle.Info.vehicleGroup],
            prim: primaryParameters[VehicleInfo.VehicleParameter.Magazine],
            sec: secondaryParameters[VehicleInfo.VehicleParameter.Magazine]);
        ...
        {
            MenuController.SetActionBoxType(ActionBoxType.Deliver);
        ...
        {
            MenuController.SetActionBoxType(ActionBoxType.Installed);
            return;
        }
    ...
    private void InstantiateUpgradeArrow(UpgradeButtons upgradesButtons)
    {
        ...
        var aligner = upgradesButtons.SprUpgrade.GetComponent<UniAligner>();
        aligner.objectsToAlign.Clear();
        aligner.objectsToAlign = new List<UniAligner.UniAlignerItem>(2);
        aligner.alignOn = upgradesButtons.bgForAligment;
        aligner.AddItem(upgradesButtons.lblText.GetComponent<Renderer>(), 0, 0);
        aligner.AddItem(upgradesButtons.SprUpgrade.GetComponent<Renderer>(), rangeBetweenIconAndArrow, 0);
        aligner.position = UniAligner.Positions.Center;
        aligner.Align();
    ...
    private void OnDeliverClick(string buttonName)
    {
        ...
        DeliverModule(
            ...
            finishCallback: delegate (Http.Response response, bool result)
            {
                ...
                ModuleReceived(Shop.CurrentVehicle, ModuleInView.type);
    ...
    public void ModuleReceived(UserVehicle vehicle, TankModuleInfos.ModuleType moduleType)
    {
        if (Shop.CurrentVehicle.Info.id == vehicle.Info.id &&
            ...
            GUIPager.ActivePage == GuiPageName)
        {
        ...
        MenuController.ModuleInstallSound();
    }
