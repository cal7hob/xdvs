...
public class RefillGasTank : MonoBehaviour
{
    ...
    {
        if (GUIPager.ActivePage != "RefillGasTank") {
            return;
        }
    ...
    public void ShowRefillGasTankWindow()
    {
        ...
        GUIPager.SetActivePage("RefillGasTank", true, !GameData.IsGame(Game.CodeOfWar));
        HelpTools.SetTextToAllLabelsInCollection(lblRefillAllGas, Localizer.GetText("lblRefillGasTank", ProfileInfo.GoldForFullFuelTank));
    ...
    private void BuyFuel()
    {
        ...
        if (ProfileInfo.Gold >= ProfileInfo.GoldForFullFuelTank)
        {
            if (m_isBusy)
            {
