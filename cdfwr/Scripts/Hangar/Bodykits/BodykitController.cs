...
public class BodykitController : MonoBehaviour
{
    ...
    public Transform ShowGun(Decal decal, SoldierController soldier)
    {
        foreach (WeaponKit weaponKit in GetComponentsInChildren<WeaponKit>(true))
        {
            if (weaponKit != null)
            {
                if (weaponKit.TryActivate(decal, soldier))
                {
                    gun = weaponKit.transform;
                }
            }
        }
        return gun;
    ...
    public void DrawDecal(Decal decal)
    {
        if (decal == null)
        {
            Debug.Log("null decal?");
            return;
        }
        HangarWeaponsHolder.Instance.SetActiveWeaponById(decal.id);
    ...
    private List<Material> CollectPaintableMaterials(Shader targetShader, bool searchInShared)
    {
        ...
        foreach (Renderer childRenderer in renderers)
        {
            ...
            if (childRenderer.GetComponentInParent<WeaponKit>())
            {
                continue;
            }
