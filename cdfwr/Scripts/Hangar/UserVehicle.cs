...
public class UserVehicle
{
    ...
    {
        get { return parameters[VehicleInfo.VehicleParameter.ReloadTime]; }
    }
    public float Magazine
    {
        get { return parameters[VehicleInfo.VehicleParameter.Magazine]; }
    }
    ...
    {
        Dictionary<VehicleInfo.VehicleParameter, ObscuredFloat> _parameters = new Dictionary<VehicleInfo.VehicleParameter, ObscuredFloat>(8);

        _parameters.Add(VehicleInfo.VehicleParameter.Magazine, (float)info.baseMagazine);
        _parameters.Add(VehicleInfo.VehicleParameter.ReloadTime, (float)info.baseReloadTime);
    ...
    public void PurschaseDecalById(int decalId)
    {
        upgrades.PurchaseDecal(DecalPool.Instance.GetItemById(decalId));
    }
    ...
    private void ApplyBodykitEffect(Dictionary<VehicleInfo.VehicleParameter, ObscuredFloat> _params, Bodykit bodykit)
    {
        ...
        _params[VehicleInfo.VehicleParameter.Magazine] += bodykit.magazineGain;// * Info.baseIRCMROF;
        _params[VehicleInfo.VehicleParameter.ReloadTime] += bodykit.reloadGain;// * Info.baseIRCMROF;
    }
    ...
    private void ExpiredCamouflageRemoving()
    {
        ...
        if (upgrades.OwnedCamouflages.TryToTakeAwayById(upgrades.CamouflageId, out nextCamoId))
        {
            ...
            upgrades.SetCamouflageId(nextCamoId);

            if (GUIPager.ActivePage != "PatternShop")
    ...
    private void ExpiredDecalRemoving()
    {
        ...
        if (upgrades.OwnedDecals.TryToTakeAwayById(upgrades.DecalId, out nextDecalId))
        {
            ...
            upgrades.SetDecalId(nextDecalId);

            if (GUIPager.ActivePage != "DecalShop")
            {
