...
public class GoToVehicleShopTutorial : Tutorial
{
    ...
    public static bool HasOtherVehicles
    {
        get
        {
            var vehiclesCount = ProfileInfo.vehicleUpgrades.Count(vehicle => vehicle.Value.vehicleId != 21); // Временный костыль. Вообще у нас пока нет FreeVehicle
    ...
    {
        var position
            = GameData.IsGame(Game.CodeOfWar)
